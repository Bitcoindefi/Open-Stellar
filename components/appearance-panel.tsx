  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      if (canvas && ctx) {
        ...
        tickRef.current += 1;
        const previewAgent: MoltbotAgent = {
          ...agent,
          pixelX: 50,
          pixelY: 22,
          direction: "right",
...
export function AppearancePanel({
    ...
    async (skinId: SkinId, level: number, agentId: string) => {
      ...
      try {
        const res = await fetch(
          `/api/agents/${encodeURIComponent(agentId)}/appearance`,
          {
            method: "POST",
        ...
        toast.success(
          `Equipped ${SKINS.find((s) => s.id === skinId)?.name} skin`,
        );
      } catch (e) {
  ...
  const handleToggleAccessory = useCallback(
    async (
      ...
      try {
        const res = await fetch(
          `/api/agents/${encodeURIComponent(agent.id)}/appearance`,
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              action: "equip-accessories",
              accessoryIds: next,
              badgeIds,
            }),
          },
        ...
        const data = await res.json();
        if (!data.ok)
          throw new Error(data.error || "Could not update accessories");
        onUpdateAgentAppearance(agent.id, data.appearance);
      } catch (e) {
        const msg =
          e instanceof Error ? e.message : "Could not update accessories";
        setError(msg);
  ...
  const handleBuyColor = useCallback(
    async (agent: MoltbotAgent) => {
      ...
      if (!treasuryAddress) {
        setError(
          "Color purchases are disabled -- treasury address not configured",
        );
        return;
      }
      if (!agent.wallet?.funded) {
        setError(
          "Agent needs a funded Stellar wallet to pay for color changes -- assign one in the Wallet tab",
        );
        return;
      }
      setLoading("color");
      ...
      try {
        ...
        if (!appearanceData.ok)
          throw new Error(appearanceData.error || "Could not save new color");
    ...
    >
            ...
            return (
              <button
                ...
                title={
                  unlocked
                    ? skin.description
                    : `Requires level ${skin.levelRequired}`
                }
                style={{
                  textAlign: "left",
                  padding: "8px 10px",
      ...
      <div>
        ...
        >
          {ACCESSORIES.map((accessory) => {
            ...
            const equipped = agent.appearance.accessories.includes(
              accessory.id,
            );
            ...
            return (
              <button
                ...
                onClick={() =>
                  handleToggleAccessory(accessory.id, agent, badgeIds)
                }
                title={
                  unlocked ? accessory.name : `Requires badge: ${badge?.name}`
                }
                style={{
                  display: "flex",
                  alignItems: "center",
      ...
      <div>
        ...
        <button
          ...
          disabled={
            loading !== null ||
            colorInput.toLowerCase() === agent.color.toLowerCase()
          }
          style={{
            marginTop: 8,
            width: "100%",
        ...
        >
          {loading === "color"
            ? "Processing payment..."
            : `Set Color — ${COLOR_CHANGE_COST_XLM} XLM`}
        </button>
        <div
          style={{
            fontFamily: "monospace",
