...
const EXPLORER = (id: string) =>
  `https://stellar.expert/explorer/${currentNetwork()}/contract/${id}`;
...
export function PassportPanel() {
  ...
  const addLog = (line: string) =>
    setLog((l) =>
      [...l, `${new Date().toLocaleTimeString()}  ${line}`].slice(-40),
    );
    ...
    getFreighter().then(async (f) => {
      ...
      try {
        ...
        if (result) {
          ...
          const pk =
            typeof pkResult === "string" ? pkResult : pkResult?.publicKey;
  ...
  async function doMint() {
    ...
    setPayRes(null);
    addLog(
      `> generating witness + Groth16 proof client-side (cap ${cap} XLM)…`,
    );
    try {
      ...
      addLog(
        `+ proof generated in ${m.provingMs} ms · off-chain verify: ${m.offChainValid}`,
      );
  ...
  async function doVerify() {
    ...
    setCommitResult(null);
    addLog(
      `> simulating proof against AgentPassportValidator (read-only, no gas)…`,
    );
  ...
  async function doCommit() {
      ...
      try {
        ...
        key =
          typeof accessResult === "string"
            ? accessResult
            : accessResult?.address || accessResult?.publicKey;
    ...
    addLog(
      `> committing passport on Soroban (signing with ${key.slice(0, 8)}…)`,
    );
    try {
      ...
      if (r.ok)
        setCollection(
          savePassport({ ...minted, txHash: r.hash, proof: minted }),
        );
      addLog(
        r.ok
  ...
  async function doPay() {
    ...
    setPaying(true);
    addLog(
      `> agent #${minted?.agentId} requests payment of ${payAmount} XLM (x402 gate)…`,
    );
    ...
    const reason = authorized
      ? "Within proven spend cap"
      : "Exceeds proven spend cap";
  ...
  async function doReplay() {
    ...
    setReplay(r);
    addLog(
      r.ok ? `! unexpectedly accepted` : `+ chain rejected replay — ${r.error}`,
    );
    setReplaying(false);
  }
    ...
    <TooltipPrimitive.Provider delayDuration={200}>
      <div className="grid gap-5 xl:grid-cols-[1.1fr_0.9fr]">
        ...
        <div className="space-y-4">
            ...
            {PASSPORT_STEPS.map((label, i) => (
              ...
              >
                <div className="flex w-full items-center">
                  {i > 0 && (
                    <div
                      className={`h-px flex-1 transition-colors ${i <= reachedIdx + 1 ? "bg-cyan-500/60" : "bg-slate-800"}`}
                    />
                  )}
                  <div
                  ...
                  {i < PASSPORT_STEPS.length - 1 && (
                    <div
                      className={`h-px flex-1 transition-colors ${i < reachedIdx ? "bg-cyan-500/60" : "bg-slate-800"}`}
                    />
                  )}
                </div>
                <span
                  className={`text-[9px] uppercase tracking-[0.18em] ${i <= reachedIdx ? "text-cyan-400" : i === reachedIdx + 1 ? "text-slate-400" : "text-slate-700"}`}
                >
                  {label}
                </span>
              </div>
          ...
          <Step
            ...
            subtitle={
              <>
                ...
                <ZKTerm
                  term="Groth16"
                  tip="Zero-knowledge proof system. Groth16 generates a 192-byte proof off-chain that is verifiable in milliseconds on-chain."
                />{" "}
                proof — keys never leave the browser
              </>
            }
          >
            ...
            {minted && (
              <div className="mt-4 grid gap-2 sm:grid-cols-2">
                ...
                <Fact
                  label="Off-chain valid"
                  value={minted.offChainValid ? "true" : "false"}
                />
                <Fact
                  label="Nullifier"
                  value={short(minted.nullifierHash)}
                  mono
                />
              </div>
            )}
          ...
          <Step
            n="02"
            title="Simulate on-chain verification"
            subtitle={
              <>
                <ZKTerm
                  term="BN254"
                  tip="Barreto-Naehrig elliptic curve. BN254 enables efficient pairing operations used to verify Groth16 proofs in under 1ms on-chain."
                />{" "}
                pairing checked by the{" "}
                <ZKTerm
                  term="Soroban"
                  tip="Stellar's smart contract platform. Soroban runs WebAssembly contracts with deterministic execution and Rust-friendly tooling."
                />{" "}
              ...
              >
                {verifyRes.ok && verifyRes.attestation ? (
                  <div className="grid gap-2 sm:grid-cols-2">
                    <Fact
                      label="Status"
                      value="SIMULATION PASSED"
                      tone="text-amber-300"
                    />
                    <Fact
                      label="Ledger (sim)"
                      value={String(verifyRes.attestation.ledger)}
                    />
                    <Fact
                      label="Proven cap"
                      value={`${fromStroops(verifyRes.attestation.spend_cap)} XLM`}
                    />
                    <Fact
                      label="Registry root"
                      value={short(verifyRes.attestation.registry_root)}
                      mono
                    />
                  </div>
                ) : (
                  <p className="font-mono text-sm text-rose-300">
                    Rejected — {verifyRes.error}
                  </p>
                )}
              </div>
            )}
          ...
          {verifyRes?.ok && (
            <Step
              ...
              subtitle={
                <>
                  Sign with Freighter to persist the passport on{" "}
                  <ZKTerm
                    term="Soroban"
                    tip="Stellar's smart contract platform. Writing to it requires signing a transaction with your wallet."
                  />{" "}
                  (costs gas)
                </>
              }
                ...
                >
                  {commitResult.ok ? (
                    <div className="grid gap-2 sm:grid-cols-2">
                      <Fact
                        label="Status"
                        value="REGISTERED ON-CHAIN"
                        tone="text-emerald-300"
                      />
                      {commitResult.hash && (
                        <Fact
                          label="Tx hash"
                          value={short(commitResult.hash, 16)}
                          mono
                        />
                      )}
                    </div>
          ...
          <Step
            n="03"
            title="Authorize x402 payment"
            subtitle="Settles only if amount ≤ proven spend cap"
          >
            <div className="flex flex-wrap items-end gap-4">
              <label className="flex-1 min-w-[180px]">
                <span className="text-[10px] uppercase tracking-[0.28em] text-slate-500">
                ...
                />
                <span className="mt-1 block font-mono text-sm text-amber-200">
                  {payAmount} XLM requested
                </span>
              </label>
              <button
                type="button"
            ...
            {payRes && (
              ...
              >
                {payRes.authorized ? "✓ APPROVED" : "✗ DENIED"} ·{" "}
                {payRes.amount} XLM — {payRes.reason}
              </p>
            )}
          </Step>
          ...
          <Step
            ...
            subtitle="A real previously-spent proof is rejected on-chain (NullifierUsed)"
          >
            <button
              type="button"
              onClick={doReplay}
            ...
            {replay && (
              <p
                className={`mt-4 font-mono text-sm ${replay.ok ? "text-rose-300" : "text-emerald-300"}`}
              >
                {replay.ok
                  ? "! unexpectedly accepted"
                  : `✓ chain rejected replay — ${replay.error}`}
              </p>
            )}
          </Step>
        </div>
        ...
        <div className="space-y-4">
          <PassportCardMini
            cap={minted?.spendCap ?? toStroops(cap)}
            minted={minted}
            verified={!!verifyRes?.ok}
            committed={committed}
          />
          <PassportCollectionPanel
            collection={collection}
            onRevoke={(passport) => {
              setCollection(revokePassportLocal(passport));
              addLog(
                `+ locally marked passport ${short(passport.nullifierHash, 12)} as revoked`,
              );
            }}
          />
          ...
          <div className="rounded-[28px] border border-slate-800 bg-slate-950/80 p-5 space-y-3">
            <p className="text-[10px] uppercase tracking-[0.32em] text-slate-500">
              Deployed contracts ({currentNetwork()})
            </p>
            <ContractRow
  ...
  return (
    <section className="rounded-[28px] border border-slate-800 bg-slate-950/80 p-5 shadow-[0_24px_80px_rgba(2,8,23,0.45)] backdrop-blur">
      <div className="flex items-start gap-3">
        ...
        <div>
          <h3 className="font-pixel text-sm uppercase text-slate-100">
            {title}
          </h3>
          <p className="mt-1 font-vt323 text-lg leading-5 text-slate-400">
            {subtitle}
          </p>
        </div>
      </div>
...
function Fact({
  ...
  return (
    <div className="rounded-2xl border border-slate-800 bg-[#09101a] p-3">
      ...
      <p
        className={`mt-1 text-sm ${mono ? "font-mono" : "font-mono"} ${tone ?? "text-slate-200"}`}
      >
        {value}
      </p>
    </div>
  ...
  if (!collection || collection.passports.length === 0) {
    return (
      <div className="rounded-[28px] border border-slate-800 bg-slate-950/80 p-5">
        ...
        <p className="mt-3 font-vt323 text-lg text-slate-400">
          No persisted passports yet. Register one to start history.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-[28px] border border-slate-800 bg-slate-950/80 p-5">
      ...
      <p className="mt-2 font-mono text-[11px] text-slate-400">
        Agent #{collection.agentId} · primary{" "}
        {short(collection.primaryPassport, 18)}
      </p>
      <div className="mt-4 space-y-2">
        {collection.passports.map((passport) => {
          ...
          const expiresInDays = Math.max(
            0,
            Math.ceil(expiresMs / (24 * 60 * 60 * 1000)),
          );
          const active = passport.status === "ACTIVE";
          return (
            <div
              key={passport.id}
              className="rounded-2xl border border-slate-800 bg-[#09101a] p-3"
            >
              <div className="flex items-center justify-between gap-3">
                <div>
                  <p className="font-mono text-xs text-slate-200">
                    {short(passport.nullifierHash, 18)}
                  </p>
                  <p className="mt-1 text-[10px] uppercase tracking-[0.2em] text-slate-500">
                    {active
                      ? `${expiresInDays}d until expiry`
                      : passport.status}{" "}
                    · {fromStroops(passport.spendCap)} XLM
                  </p>
                </div>
                <span
                  className={`font-mono text-[10px] ${active ? "text-emerald-300" : passport.status === "REVOKED" ? "text-rose-300" : "text-amber-300"}`}
                >
                  {passport.status}
                </span>
              </div>
    ...
    <div className="rounded-[28px] border border-cyan-500/20 bg-gradient-to-br from-slate-950 to-[#06101c] p-5 shadow-[0_24px_80px_rgba(2,8,23,0.55)]">
      ...
      <div className="mt-6 grid grid-cols-2 gap-3">
        <CardStat
          ...
          label="Agent"
          value={minted ? `#${minted.agentId}` : "—"}
        />
        <CardStat
          icon={<Coins className="h-3.5 w-3.5" />}
          label="Spend cap"
          value={`${fromStroops(cap)} XLM`}
        />
        <CardStat
          icon={<Fingerprint className="h-3.5 w-3.5" />}
          label="Nullifier"
          value={minted ? short(minted.nullifierHash, 10) : "—"}
        />
        <CardStat
          icon={<Activity className="h-3.5 w-3.5" />}
          label="Proof"
          value={minted ? `${minted.provingMs}ms` : "—"}
        />
      </div>
      <p className="mt-5 font-vt323 text-base leading-5 text-slate-400">
        Owner key &amp; real balance never leave the browser — only the proof
        and its four public inputs go on-chain.
      </p>
    </div>
  );
}
