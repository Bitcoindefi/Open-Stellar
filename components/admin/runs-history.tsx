  ...
  async function rerun(runId: string) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setRerunMessage(
          `Queued ${data.run.id}; estimated cost ${data.estimate.estimatedCostXlm} XLM (${data.estimate.deltaXlm} delta).`,
        );
      }
    } finally {
      setLoadingRunId(null);
    ...
    <main className="min-h-screen bg-[#04070d] px-4 py-6 text-slate-100 sm:px-6 lg:px-8">
      <div className="mx-auto grid max-w-7xl gap-5">
        <header className="rounded-[28px] border border-cyan-500/20 bg-slate-950/80 p-5 shadow-[0_24px_80px_rgba(2,8,23,0.5)]">
          <Link
            href="/admin"
            className="inline-flex items-center gap-2 rounded-full border border-slate-800 bg-slate-950 px-3 py-2 text-xs uppercase tracking-[0.2em] text-slate-300 transition hover:border-cyan-400/40 hover:text-cyan-200"
          >
          ...
          <div className="mt-5 grid gap-4 lg:grid-cols-[1fr_auto] lg:items-end">
            <div>
              ...
              <h1 className="mt-3 font-pixel text-2xl uppercase text-cyan-100 sm:text-3xl">
                Orchestration Runs
              </h1>
              <p className="mt-3 max-w-3xl font-vt323 text-xl leading-7 text-slate-300">
                Review completed and failed multi-agent workflows, inspect every
                step payload, and queue a re-run with a cost delta before
                dispatch.
              </p>
            </div>
            {rerunMessage && (
            ...
            <tbody>
              {runs.map((run) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 font-mono">
                    {run.totalCostXlm} XLM
                  </td>
                  <td className="px-4 py-3">
                    <span
                      className={`rounded-full border px-2.5 py-1 text-[10px] uppercase tracking-[0.18em] ${statusClasses(run.status)}`}
                    >
                      {run.status}
                    </span>
                  </td>
          ...
          <section className="rounded-[28px] border border-cyan-500/20 bg-slate-950 p-5 shadow-[0_24px_80px_rgba(2,8,23,0.55)]">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
              <div>
                ...
                <h2 className="mt-2 font-pixel text-xl uppercase text-cyan-100">
                  {selected.run.id}
                </h2>
                <p className="mt-2 max-w-3xl font-vt323 text-xl leading-7 text-slate-300">
                  {selected.run.goal}
                </p>
              </div>
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-3">
              <Stat
                label="Original cost"
                value={`${selected.rerunEstimate.originalCostXlm} XLM`}
              />
              <Stat
                label="Re-run estimate"
                value={`${selected.rerunEstimate.estimatedCostXlm} XLM`}
              />
              <Stat
                label="Delta"
                value={`${selected.rerunEstimate.deltaXlm} XLM`}
              />
            </div>
            ...
            <div className="mt-5 grid gap-3">
              {selected.run.steps.map((step, index) => (
                <article
                  key={step.id}
                  className="rounded-[22px] border border-slate-800 bg-[#09101a] p-4"
                >
                  <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                    <div>
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <span
                          className={`rounded-full border px-2.5 py-1 text-[10px] uppercase tracking-[0.18em] ${statusClasses(step.status)}`}
                        >
                          {step.status}
                        </span>
                        {step.dependsOn && (
                      ...
                      <h3 className="mt-3 font-pixel text-sm uppercase text-slate-100">
                        {step.task}
                      </h3>
                    ...
                    <div className="font-mono text-xs text-slate-400">
                      {formatDuration(step.durationMs)} / {step.costXlm ?? "-"}{" "}
                      XLM
                    </div>
                  </div>
                  ...
                  <div className="mt-4 grid gap-3 lg:grid-cols-3">
                    ...
                    <JsonBlock
                      title="Output"
                      value={step.result ?? { status: "pending" }}
                    />
                    <div className="rounded-2xl border border-slate-800 bg-slate-950/70 p-3">
                      ...
                      {step.receiptId && (
                        <a
                          href={`/explorer?q=${encodeURIComponent(step.receiptId)}`}
