...
type AdminConsoleProps = Readonly<{
  agents: MoltbotAgent[];
  districts: District[];
}>;
...
const plans: Plan[] = [
  ...
  },
  {
    ...
    features: [
      ...
      "Priority relay lanes",
      "Usage and policy controls",
    ],
...
interface OverviewTabProps {
  ...
  readonly districtCards: Array<{
    district: District;
    squad: MoltbotAgent[];
    working: number;
    avgLoad: number;
  }>;
  readonly topAgents: MoltbotAgent[];
  readonly selectedPlan: Plan;
  readonly setSelectedPlan: (plan: Plan) => void;
}
...
function OverviewTab({
  ...
  return (
    <>
      <section className="grid gap-5 xl:grid-cols-[0.8fr_1.2fr_0.8fr]">
        ...
        >
          ...
          <div className="rounded-2xl border border-slate-800 bg-slate-950/70 p-4">
            ...
            <p className="mt-3 font-vt323 text-lg leading-6 text-slate-300">
              Monthly subscription buys access to orchestration control plus the
              x402 payment extension. Request volume is the primary limiter, not
              seats.
            </p>
          </div>
        </Panel>
        ...
        >
          {districtCards.map(({ district, squad, working, avgLoad }) => (
            <div
              key={district.id}
              className="rounded-[22px] border border-slate-800 bg-slate-950/70 p-4"
            >
              <div className="flex items-start justify-between gap-3">
                <div>
                  <p className="font-pixel text-sm uppercase text-slate-100">
                    {district.name}
                  </p>
              ...
              <p className="mt-4 font-vt323 text-lg text-slate-300">
                Suggested as an isolated customer squad with policy-based
                request caps and shared payment settlement.
              </p>
            </div>
          ))}
        </Panel>
        ...
        >
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <p
                      className={`font-pixel text-sm uppercase ${plan.accent}`}
                    >
                      {plan.name}
                    </p>
          ...
          <div className="rounded-[22px] border border-slate-800 bg-[#09101a] p-4">
            ...
            <h3 className="mt-3 font-pixel text-base uppercase text-cyan-100">
              {selectedPlan.name}
            </h3>
            <ul className="mt-4 space-y-2">
              {selectedPlan.features.map((feature) => (
                <li
                  key={feature}
                  className="flex items-center gap-2 font-vt323 text-lg text-slate-300"
                >
      ...
      <Panel
        title="Active x402 subscriptions"
        eyebrow="Admin billing console"
          ...
          <table className="w-full min-w-[760px] text-left">
            ...
            <tbody>
              {subscriptions.map((subscription) => (
                <tr
                  key={`${subscription.agent}-${subscription.service}`}
                  className="border-b border-slate-900/80 font-mono text-xs text-slate-300"
                >
                  ...
                  <td className="py-3">
                    {subscription.used.toLocaleString()} /{" "}
                    {subscription.limit.toLocaleString()}
                  </td>
                  <td className="py-3">{subscription.renewsAt}</td>
                  <td className="py-3 text-emerald-300">{subscription.mrr}</td>
                  <td className="py-3">
                    <span
                      className={`rounded-full border px-2 py-1 text-[10px] uppercase tracking-[0.18em] ${subscription.status === "active" ? "border-emerald-400/30 bg-emerald-400/10 text-emerald-300" : "border-amber-400/30 bg-amber-400/10 text-amber-300"}`}
                    >
                      {subscription.status}
                    </span>
                  </td>
      ...
      <section className="grid gap-5 xl:grid-cols-[1fr_0.9fr]">
        <Panel
          title="Top operators"
          eyebrow="Main page agents"
          bodyClassName="grid gap-3 md:grid-cols-2 xl:grid-cols-1"
        >
          {topAgents.map((agent) => (
            <div
              key={agent.id}
              className="rounded-[20px] border border-slate-800 bg-slate-950/70 p-4"
            >
        ...
        <Panel
          title="Offer framing"
          eyebrow="What this screen is selling"
    ...
    <section className="rounded-[28px] border border-cyan-500/20 bg-slate-950/60 p-5">
      <div className="mb-5 max-w-3xl">
        ...
        <p className="mt-3 font-vt323 text-xl leading-7 text-slate-300">
          Each agent mints a ZK passport (Groth16 / Soroban) proving it is
          backed by a verified human and is solvent for its spend cap. The x402
          settlement rail then releases a payment only when the agent holds a
...
export function AdminConsole({ agents, districts }: AdminConsoleProps) {
  ...
  const activeAgents = agents.filter(
    (agent) => agent.status === "active" || agent.status === "working",
  );
  ...
  const avgCpu = Math.round(
    agents.reduce((sum, agent) => sum + agent.cpu, 0) /
      Math.max(1, agents.length),
  );
  const avgMemory = Math.round(
    agents.reduce((sum, agent) => sum + agent.memory, 0) /
      Math.max(1, agents.length),
  );
  ...
  const subscriptionMrr = subscriptions.reduce(
    (sum, subscription) => sum + Number(subscription.mrr.split(" ")[0]),
    0,
  );
  ...
  const districtCards = districts.map((district) => {
    ...
    const avgLoad = Math.round(
      squad.reduce((sum, agent) => sum + agent.cpu, 0) /
  ...
  const renderActiveTab = () => {
    switch (tab) {
      ...
      case "passport":
        return <PassportTab />;
      case "private-deploy":
        return <PrivateDeployTab />;
      case "cloud-agents":
        return <CloudAgentsTab />;
      default:
        return (
          <OverviewTab
            avgCpu={avgCpu}
    ...
    <main className="min-h-screen overflow-hidden bg-[#04070d] text-slate-100">
      ...
      <div className="relative mx-auto flex min-h-screen w-full max-w-[1500px] flex-col gap-5 px-4 py-4 sm:px-6 lg:px-8">
          ...
          <section className="rounded-[28px] border border-cyan-500/20 bg-slate-950/80 p-5 shadow-[0_0_0_1px_rgba(15,23,42,0.7),0_24px_80px_rgba(2,8,23,0.55)] backdrop-blur">
            <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
              <div className="max-w-3xl">
                ...
                <p className="mt-4 max-w-2xl font-vt323 text-xl leading-7 text-slate-300">
                  The admin view now matches the city control surface: same mock
                  squads, same districts, and the same operating tone. Here you
                  sell x402 payment rails, API access, and request-capped
                  orchestration as a managed layer on top of the main
                  simulation.
                </p>
              </div>

              <div className="rounded-[24px] border border-cyan-400/20 bg-cyan-950/20 p-4 lg:w-80">
                <p className="text-[10px] uppercase tracking-[0.32em] text-cyan-300">
                  Managed revenue
                </p>
                <div className="mt-3 flex items-baseline justify-between">
                  <span className="font-pixel text-2xl text-cyan-100">
                    {subscriptionMrr} XLM
                  </span>
                ...
                <p className="mt-3 font-vt323 text-lg leading-6 text-slate-300">
                  Recurring x402 subscriptions anchored to district workloads
                  and tenant API access.
                </p>
              </div>
            </div>
            ...
            <div className="mt-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
              ...
              <SignalStat
                ...
                value={formatCompact(totalTasks)}
              ...
              <SignalStat
                ...
                value={`${remaining} reqs`}
                color="text-violet-200"
              />
            </div>
          </section>

          <section className="flex flex-col justify-between rounded-[28px] border border-slate-800 bg-slate-950/80 p-5 shadow-[0_24px_80px_rgba(2,8,23,0.45)] backdrop-blur">
            <div>
              <div className="flex items-center justify-between gap-3">
                <p className="text-[10px] uppercase tracking-[0.32em] text-slate-400">
                  Admin API Key
                </p>
                <span className="rounded-full border border-emerald-400/30 bg-emerald-400/10 px-2.5 py-1 text-[10px] uppercase tracking-[0.2em] text-emerald-300">
                  Ready to test
                </span>
              </div>
              <p className="mt-3 font-mono text-xs text-cyan-300 break-all">
                {demoKey}
              </p>
              <p className="mt-2 font-vt323 text-lg leading-6 text-slate-400">
                Auto-generated admin key for local simulation and tenant
                provisioning.
              </p>
            </div>
            ...
            <div className="mt-5 flex flex-wrap gap-2">
              <button
                type="button"
                onClick={handleCopy}
                className="inline-flex items-center gap-2 rounded-full border border-cyan-400/40 bg-cyan-400/10 px-4 py-2 text-xs uppercase tracking-[0.2em] text-cyan-200 transition hover:bg-cyan-400/20"
              >
                {copied ? (
              ...
              <a
                href="/admin/keys"
                className="inline-flex items-center gap-2 rounded-full border border-cyan-400/50 bg-cyan-500/10 px-4 py-2 text-xs uppercase tracking-[0.2em] text-cyan-200 shadow-[0_0_15px_rgba(6,182,212,0.2)] transition hover:border-cyan-400 hover:bg-cyan-500/20"
              >
        ...
        >
          ...
          <TabButton
            active={tab === "private-deploy"}
            onClick={() => setTab("private-deploy")}
            icon={<Rocket className="h-3.5 w-3.5" />}
          >
            Private Deploy
          </TabButton>
          <TabButton
            active={tab === "cloud-agents"}
            onClick={() => setTab("cloud-agents")}
...
function ReceiptsTab() {
  ...
  const renderReceiptsBody = () => {
    ...
    if (receipts.length === 0) {
      return (
        <p className="px-4 py-6 font-vt323 text-xl text-slate-400">
          No x402 receipts have been recorded yet.
        </p>
      );
    }
    return receipts.map((receipt) => (
      <div
        key={receipt.id}
        className="grid grid-cols-[1fr_1fr_0.8fr_0.9fr] gap-3 border-b border-slate-900 px-4 py-3 font-mono text-xs text-slate-300 last:border-b-0 md:grid-cols-[1fr_1fr_0.7fr_0.9fr_1.2fr]"
      >
  ...
  return (
    <section className="space-y-4 rounded-[28px] border border-cyan-500/20 bg-slate-950/70 p-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="font-pixel text-xl uppercase text-cyan-100">
            x402 Settlement Receipts
          </h2>
          <p className="mt-1 font-vt323 text-lg text-slate-300">
            Audit trail of micropayments and settled quote receipts stored on
            this node.
          </p>
        </div>
        <button
          ...
          onClick={() => {
            ...
            fetch("/api/protocol/x402/receipts?pageSize=50")
              ...
              .then((data) => {
                setReceipts(Array.isArray(data.receipts) ? data.receipts : []);
                setStatus(
                  data.ok ? "" : data.error || "Failed to load receipts",
                );
              })
              .catch(() => setStatus("Failed to load receipts"));
          }}
          className="inline-flex items-center gap-2 rounded-full border border-cyan-400/40 bg-cyan-400/10 px-4 py-2 text-xs uppercase tracking-[0.2em] text-cyan-200 hover:bg-cyan-400/20"
        >
          Refresh
        </button>
      </div>
...
const API_ENDPOINTS = [
  {
    method: "POST",
    path: "/api/protocol/x402/quote",
    desc: "Create x402 payment quote",
  },
  {
    method: "POST",
    path: "/api/protocol/x402/settle",
    desc: "Settle x402 payment (optional passport gate)",
  },
  {
    method: "POST",
    path: "/api/protocol/x402/subscriptions",
    desc: "Create recurring x402 API subscription",
  },
  {
    method: "GET",
    path: "/api/protocol/x402/subscriptions/:agentId/:serviceId",
    desc: "Check active subscription access",
  },
  {
    method: "POST",
    path: "/api/protocol/passport/authorize",
    desc: "ZK spend-cap authorization gate",
  },
  {
    method: "GET",
    path: "/api/protocol/passport/status",
    desc: "On-chain agent passport lookup",
  },
  {
    method: "GET",
    path: "/api/protocol/reputation",
    desc: "Agent reputation query",
  },
  {
    method: "POST",
    path: "/api/protocol/reputation",
    desc: "Record reputation action",
  },
  {
    method: "GET",
    path: "/api/protocol/track8004",
    desc: "ERC-8004 agent identity resolution",
  },
  {
    method: "GET",
    path: "/api/stellar/balance",
    desc: "Stellar account balance",
  },
  {
    method: "POST",
    path: "/api/stellar/build-tx",
    desc: "Build and sign Stellar transaction",
  },
  {
    method: "POST",
    path: "/api/stellar/submit-tx",
    desc: "Submit signed transaction",
  },
  {
    method: "POST",
    path: "/api/stellar/fund",
    desc: "Friendbot testnet funding",
  },
] as const;
...
const ENV_VARS = [
  {
    ...
    example: "My Open Stellar Node",
    desc: "Display name in the admin console header",
  },
  {
  ...
  },
  {
    ...
    example: "abc123…",
    desc: "WalletConnect Cloud project ID",
  },
  {
    name: "ADMIN_API_KEY",
    example: "osk_…",
    desc: "Admin API key (auto-generated on first boot if unset)",
  },
  {
    name: "NEXT_PUBLIC_APP_URL",
    example: "https://your-instance.vercel.app",
    desc: "Public URL of your deployment",
  },
] as const;
    ...
    <>
      <section className="rounded-[28px] border border-amber-500/20 bg-slate-950/80 p-5 shadow-[0_24px_80px_rgba(2,8,23,0.45)] backdrop-blur">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
          <div className="max-w-3xl">
            ...
            <p className="mt-4 max-w-2xl font-vt323 text-xl leading-7 text-slate-300">
              Fork the repository, configure your Stellar and x402 credentials,
              and deploy to Vercel in under five minutes. Your instance runs the
              same payment rails, agent passport ZK layer, and reputation system
              as this one — fully isolated, fully yours.
            </p>
          </div>
      ...
      <section className="grid gap-5 xl:grid-cols-[0.65fr_1.35fr]">
        <section className="rounded-[28px] border border-slate-800 bg-slate-950/80 p-5 shadow-[0_24px_80px_rgba(2,8,23,0.45)] backdrop-blur">
          ...
          <h3 className="mt-3 font-pixel text-lg uppercase text-slate-100">
            Quick start
          </h3>
          <div className="mt-5 space-y-4">
            <DeployStep
              n={1}
              title="Scaffold"
              text="Run npx create-open-stellar-app my-node or fork bitcoindefi/Open-Stellar on GitHub. The repo includes ZK artifacts and Soroban bindings."
            />
            <DeployStep
              n={2}
              title="Configure"
              text="Set node name, network, and WalletConnect project ID. An admin API key is generated automatically on first boot."
            />
            <DeployStep
              n={3}
              title="Deploy"
              text="Push to main — Vercel picks it up automatically. The vercel.json enforces --webpack mode for snarkjs compatibility."
            />
          </div>
          ...
          <div className="mt-6 space-y-2">
            ...
            {ENV_VARS.map((v) => (
              <div
                key={v.name}
                className="rounded-[18px] border border-slate-800 bg-[#09101a] p-3"
              >
                <p className="font-mono text-xs text-amber-300">{v.name}</p>
                <p className="mt-1 font-mono text-[11px] text-slate-500">
                  e.g. {v.example}
                </p>
                <p className="mt-1 font-vt323 text-base text-slate-400">
                  {v.desc}
                </p>
              </div>
            ))}
        ...
        <section className="rounded-[28px] border border-slate-800 bg-slate-950/80 p-5 shadow-[0_24px_80px_rgba(2,8,23,0.45)] backdrop-blur">
          ...
          <p className="font-vt323 text-lg leading-6 text-slate-400">
            ...
            . Authenticate with the issued key shown on the overview panel. The
            x402 and passport endpoints run against Stellar testnet by default;
            swap the contract addresses and RPC URL in{" "}
          ...
          <div className="mt-5 space-y-2">
            {API_ENDPOINTS.map((ep) => (
              ...
              >
                ...
                <span className="hidden font-vt323 text-base text-slate-500 xl:block">
                  {ep.desc}
                </span>
              </div>
            ))}
...
function CloudAgentsTab() {
  ...
  const provision = async () => {
    ...
    setStatus(
      "Cloud agent provisioned. It will appear on the city canvas with a cloud badge.",
    );
  };
  ...
  return (
    <section className="grid gap-5 xl:grid-cols-[0.8fr_1.2fr]">
      <Panel
        title="Provision cloud agent"
        eyebrow="Vercel Edge runtime"
        bodyClassName="space-y-4"
      >
        <p className="font-vt323 text-xl leading-7 text-slate-300">
          Create an agent config and expose it at{" "}
          <span className="font-mono text-cyan-300">/agents/:agentId</span>. The
          Edge route accepts task POSTs and streams 15s SSE heartbeats.
        </p>
        <input
          value={name}
        ...
        {status ? (
          <p className="font-vt323 text-lg text-emerald-300">{status}</p>
        ) : null}
        {endpoint ? (
      ...
      <Panel
        ...
        eyebrow="Issue #21 acceptance"
        bodyClassName="grid gap-3 md:grid-cols-2"
      >
        <FeatureBlock
          title="POST tasks"
          text="The orchestrator sends JSON tasks to /agents/:agentId; the Edge Function calls Claude when ANTHROPIC_API_KEY is configured."
        />
        <FeatureBlock
          title="SSE heartbeat"
          text="GET /agents/:agentId keeps the connection open and emits heartbeat events every 15 seconds."
        />
        <FeatureBlock
          title="Canvas badge"
          text="Provisioned cloud agents are merged into the city and rendered with a CLOUD badge above the sprite."
        />
        <FeatureBlock
          title="Realtime status"
          text="Task start/completion and heartbeat updates flow through the existing health store and system event stream."
        />
      </Panel>
    </section>
  );
...
function DeployStep({
  ...
  title,
  text,
}: Readonly<{ n: number; title: string; text: string }>) {
  return (
    <div className="flex gap-4">
      ...
      <div>
        ...
        <p className="mt-1.5 font-vt323 text-lg leading-6 text-slate-400">
          {text}
        </p>
      </div>
    </div>
...
function TaskQueueTab() {
  ...
  const loadTasks = async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/tasks?includeDeadLetter=1", {
        cache: "no-store",
      });
      const data = await res.json();
      setTasks(data.tasks ?? []);
    ...
    if (deadLetter.length === 0) {
      return (
        <p className="rounded-2xl border border-emerald-500/20 bg-emerald-500/10 p-4 font-vt323 text-lg text-emerald-200">
          No dead-letter items. Failed tasks will appear here with one-click
          retry.
        </p>
      );
    }
    return deadLetter.map((task) => (
      <div
        key={task.id}
    ...
    <section className="grid gap-5 xl:grid-cols-[0.9fr_1.1fr]">
      ...
      >
        ...
        <p className="rounded-2xl border border-slate-800 bg-slate-950/70 p-4 font-vt323 text-lg leading-6 text-slate-300">
          Agents pull prioritized work by direct agent, district, or capability.
          Failures retry with exponential backoff before moving into the
          dead-letter lane.
        </p>
      </Panel>
...
function Panel({
  ...
  return (
    <section className="rounded-[28px] border border-slate-800 bg-slate-950/80 p-5 shadow-[0_24px_80px_rgba(2,8,23,0.45)] backdrop-blur">
      ...
      <h2 className="mt-3 font-pixel text-lg uppercase text-slate-100">
        {title}
      </h2>
