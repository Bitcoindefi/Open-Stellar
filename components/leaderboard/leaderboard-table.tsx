...
export function LeaderboardTable({
  ...
  const title = useMemo(
    () => (view === "week" ? "Tasks this week" : "Total tasks"),
    [view],
  );
      ...
      <div className="divide-y divide-slate-800/80">
        {agents.length === 0 && (
          <div className="px-4 py-12 text-center font-mono text-sm text-slate-400">
            No agents are registered yet. Registered agents will appear here
            within 30 seconds.
          </div>
        )}
        {agents.map((agent) => {
          ...
          return (
            ...
            >
              <div className="font-pixel text-xl text-cyan-200">
                #{agent.rank}
              </div>
              <div
              ...
              <div>
                <div className="flex flex-wrap items-center gap-2 font-pixel text-lg uppercase text-slate-100">
                  {agent.name}
                  {agent.globalRank <= 3 && (
                    <span aria-label="top-three crown">🏆</span>
                  )}
                  {agent.attestationHash && (
                    <span
                      className="rounded bg-emerald-500/10 px-1.5 py-0.5 font-mono text-[10px] uppercase text-emerald-400 border border-emerald-500/30"
                      title={`Soroban Attestation Hash: ${agent.attestationHash}`}
                    >
                      🛡️ Soroban Verified
                    </span>
                  )}
