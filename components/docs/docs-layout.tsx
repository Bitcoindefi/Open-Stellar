    ...
    <main className="min-h-screen bg-[#050816] text-slate-100">
      ...
      <div className="relative mx-auto grid max-w-7xl gap-8 px-4 py-8 md:grid-cols-[260px_1fr]">
        <aside className="md:sticky md:top-8 md:h-[calc(100vh-4rem)]">
          <Link
            href="/"
            className="mb-6 block font-mono text-xs uppercase tracking-[0.35em] text-cyan-300"
          >
            ← Open Stellar
          </Link>
        ...
        <section className="min-w-0 rounded-3xl border border-cyan-400/20 bg-slate-950/85 p-6 shadow-[0_0_80px_rgba(124,58,237,.15)] md:p-10">
          {children}
        </section>
      </div>
    </main>
...
function DocLink({
  ...
  return (
    <Link
      href={`/docs/${slug}`}
      className={`mb-1 block rounded-lg px-3 py-2 font-mono text-sm ${active ? "bg-cyan-300 text-slate-950" : "text-slate-300 hover:bg-slate-800 hover:text-cyan-200"}`}
    >
      {title}
    </Link>
  );
