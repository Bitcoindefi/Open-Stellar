  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith("```")) {
      ...
      while (++i < lines.length && !lines[i].startsWith("```"))
        code.push(lines[i]);
      nodes.push(
        <pre key={i} data-language={language || "text"}>
          <code>{code.join("\n")}</code>
        </pre>,
      );
    } else if (line.startsWith("### "))
      nodes.push(<h3 key={i}>{inline(line.slice(4))}</h3>);
    else if (line.startsWith("## "))
      nodes.push(<h2 key={i}>{inline(line.slice(3))}</h2>);
    else if (line.startsWith("# "))
      nodes.push(<h1 key={i}>{inline(line.slice(2))}</h1>);
    else if (line.startsWith("- ")) {
