...
export function WalletPanel({
  ...
  useEffect(() => {
    ...
    const update = () => {
      const remaining = Math.max(
        0,
        Math.floor(
          (new Date(x402Quote.expiresAt).getTime() - Date.now()) / 1000,
        ),
      );
    ...
    async (agent: MoltbotAgent) => {
      ...
      try {
        ...
        toast.success(`Sent ${sendAmount} XLM to ${recipient.name}`, {
          description: submitData.hash
            ? `tx: ${submitData.hash.slice(0, 18)}…`
            : undefined,
        });
    ...
    const syncProtocolState = async () => {
      try {
        const [trackRes, repRes] = await Promise.all([
          ...
          fetch(
            `/api/protocol/reputation?actorId=${encodeURIComponent(selectedAgent.id)}`,
          ),
        ]);
        ...
        if (!cancelled) {
          ...
          setReputationScore(
            typeof repData?.reputation?.score === "number"
              ? repData.reputation.score
              : null,
          );
        }
      } catch {
  ...
  const applyReputation = useCallback(
    async (agentId: string, delta: number, reason: string) => {
  ...
  const handleSettleX402 = useCallback(
    async (agent: MoltbotAgent) => {
      if (!x402Quote) return;
      ...
      try {
        ...
        setX402Quote(null);
        toast.success("x402 settled — +15 reputation", {
          description: `${x402Quote.amountUsd.toFixed(3)} USD on ${x402Quote.chain}`,
        });
      } catch (e) {
  ...
  const renderFreighterStatus = () => {
    ...
    if (!connectedKey) {
      return (
        ...
        >
          <div style={{ fontSize: 28, opacity: 0.3, fontFamily: "monospace" }}>
            XLM
          </div>
          <div
            style={{
      ...
      >
        ...
        >
          ...
          >
            ...
            />
            <span
              style={{ fontFamily: "monospace", fontSize: 9, color: "#34d399" }}
            >
              Online
            </span>
          </div>
              ...
              >
                ...
                <span
                  ...
                  style={{
                    fontFamily: "monospace",
                    fontSize: 10,
                    color: a.wallet?.funded ? "#fbbf24" : "#334155",
                  }}
                >
                  {a.wallet
                    ? `${parseFloat(a.wallet.balance).toFixed(1)} XLM`
                    : "---"}
                </span>
              </div>
            ))}
  ...
  const otherFunded = agents.filter(
    (a) => a.id !== selectedAgent.id && a.wallet?.funded,
  );
                ...
                >
                  ...
                  {otherFunded.map((a) => (
                    <option key={a.id} value={a.id}>
                      {a.name} ({truncAddr(a.wallet!.publicKey)})
                    </option>
                  ))}
                </select>
          ...
          >
            ...
            >
              ...
              <span
                style={{
                  color:
                    track8004Mode === "reputation-fallback"
                      ? "#fbbf24"
                      : "#22d3ee",
                  fontWeight: 700,
                }}
            ...
            {x402Quote && x402Countdown !== null && (
              ...
              >
                ...
                >
                  ...
                  <span
                    style={{
                      color: x402Countdown < 60 ? "#f87171" : "#64748b",
                    }}
                  >
                  ...
                  <WalletBtn
                    ...
                    onClick={async () => {
                      ...
                      try {
                        ...
                        toast("Penalty applied: -10 reputation", {
                          icon: "⚠️",
                        });
                      } catch (e) {
                        setError(
