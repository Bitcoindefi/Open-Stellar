...
export function PixelCity({
  ...
  const touchStartRef = useRef<{ x: number; y: number; moved: boolean } | null>(
    null,
  );
  ...
  const clampPan = useCallback(
    (
      ...
      if (cityW * nextZoom <= width - margin * 2) {
        ...
        x = clamp(
          x,
          width - margin - cityBounds.maxX * nextZoom,
          margin - cityBounds.minX * nextZoom,
        );
      }
      ...
      if (cityH * nextZoom <= height - margin * 2) {
        ...
        y = clamp(
          y,
          height - margin - cityBounds.maxY * nextZoom,
          margin - cityBounds.minY * nextZoom,
        );
      }
  ...
  const fitViewToContainer = useCallback(() => {
    ...
    const nextZoom = clamp(
      Math.min((rect.width - 32) / cityW, (rect.height - 96) / cityH, 1),
      MIN_ZOOM,
      1,
    );
  ...
  useEffect(() => {
    ...
    const loadedSprites: (HTMLImageElement | null)[] = new Array(
      SPRITE_CONFIGS.length,
    ).fill(null);
    const crops: ([number, number, number, number] | undefined)[] =
      SPRITE_CONFIGS.map((c) => c.crop);
  ...
  const resizeCanvas = useCallback(() => {
    ...
    if (!initializedViewRef.current) {
      ...
      setPanOffset((current) =>
        clampPan(current, zoomRef.current, rect.width, rect.height),
      );
    }
  }, [cityBounds, clampPan]);
    ...
    if (audioEngine) {
      ...
      for (const agent of agents) {
        ...
        const visualDistrict = districts.find(
          (d) =>
            agent.pixelX >= d.x &&
            agent.pixelX <= d.x + d.w &&
            agent.pixelY >= d.y &&
            agent.pixelY <= d.y + d.h,
        );
  ...
  const getPointerPoint = useCallback(
    (e: React.PointerEvent<HTMLCanvasElement>): ScreenPoint | null => {
      const canvas = canvasRef.current;
  ...
  const handlePointerMove = useCallback(
    (e: React.PointerEvent<HTMLCanvasElement>) => {
      ...
      if (pointersRef.current.size >= 2) {
        ...
        const nextZoom = clamp(
          zoomRef.current *
            (distance(first, second) / pinchRef.current.startDistance),
          MIN_ZOOM,
          MAX_ZOOM,
        );
        const nextPan = clampPan(
    ...
    >
                ...
                style={{
                  ...
                  border:
                    isFocused || isSelected
                      ? `${Math.max(1, 2 * zoom)}px solid #fbbf24`
                  ...
                  padding: 0,
                  outline: isFocused
                    ? `${Math.max(1, 2 * zoom)}px solid #22d3ee`
                    : "none",
      ...
      {hoveredAgent && tooltipPos && (
        ...
        >
          ...
          >
            <div
              style={{
                ...
                height: 7,
                borderRadius: "50%",
                background: statusColors[hoveredAgent.status] ?? "#64748b",
              }}
            />
            <span
              style={{
          ...
          >
            <div>
              {hoveredAgent.status.toUpperCase()} · CPU{" "}
              {Math.round(hoveredAgent.cpu)}%
            </div>
            {hoveredAgent.status === "offline" && (
              <div style={{ color: "#f87171" }}>
                Offline for{" "}
                {Math.floor((hoveredAgent.offlineForSeconds ?? 0) / 60)}m
              </div>
            )}
            {hoveredAgent.lastHeartbeat && (
              <div>
                Last seen{" "}
                {new Date(hoveredAgent.lastHeartbeat).toLocaleTimeString()}
              </div>
            )}
