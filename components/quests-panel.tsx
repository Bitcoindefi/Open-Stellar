...
function QuestCard({
  ...
  const isEligible =
    ...
    currentReputation >= minReputation;
  const isReputationTooLow =
    minReputation !== undefined &&
    currentReputation !== null &&
    currentReputation < minReputation;
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span
          style={{ color: "#64748b", fontSize: 10, fontFamily: "monospace" }}
        >
          {formatCountdown(quest.expiresAt)}
        </span>
      </div>
            ...
            >
              ...
              >
                <input
                  ...
                  onChange={() =>
                    onSubTaskToggle?.(quest.id, subTask.id, subTask.status)
                  }
                  style={{
                    cursor: "pointer",
                    accentColor: color,
      ...
      {hasReputationGate && (
        ...
        >
          <span
            style={{ color: "#cbd5e1", fontSize: 10, fontFamily: "monospace" }}
          >
...
export function QuestsPanel({
  ...
  const [currentReputation, setCurrentReputation] = useState<number | null>(
    null,
  );
  ...
  async function loadQuests(): Promise<void> {
    try {
      ...
      setError(
        caught instanceof Error ? caught.message : "Unable to load quests",
      );
    } finally {
      setLoading(false);
    }
    ...
    async function loadReputation(): Promise<void> {
      try {
        const response = await fetch(
          `/api/protocol/reputation?actorId=${encodeURIComponent(actorId)}`,
          { cache: "no-store" },
        );
  ...
  const handleSubTaskToggle = async (
    ...
    try {
      ...
      toast.error(
        caught instanceof Error ? caught.message : "Failed to toggle subtask",
      );
    }
  };
  ...
  const userEntry = selectedAgentId
    ? leaderboard.find((e) => e.agentId === selectedAgentId)
    : null;
  ...
  function handleClaim(quest: Quest): void {
    if (quest.reward.xlm) {
      toast.info(
        `${quest.title} requires wallet signature to claim ${quest.reward.xlm} XLM`,
      );
      return;
    }
  ...
  return (
    ...
    >
      <div style={{ padding: 12, borderBottom: "1px solid #2a3a52" }}>
        ...
        >
          {completedCount} ready to claim · resets update every minute ·{" "}
          {new Date(now).toUTCString().slice(17, 22)} UTC
        </div>
      </div>
      ...
      <div style={{ flex: 1, overflow: "auto", padding: 10 }}>
        ...
        {!loading &&
          !error &&
          (["daily", "weekly", "story"] as const).map((type) => (
            <section key={type} style={{ marginBottom: 14 }}>
              <div
                style={{
                  color: questTypeColors[type],
                  fontSize: 11,
                  fontFamily: "monospace",
                  fontWeight: 800,
                  textTransform: "uppercase",
                    ...
                    >
                      <span
                        style={{
                          ...
                          borderRadius: 5,
                          background:
                            entry.rank <= 3 ? "#0f172a" : "transparent",
                          color: rankColor,
                          fontSize: 11,
                          fontFamily: "monospace",
                          fontWeight: 800,
                      ...
                      <div style={{ minWidth: 0, flex: 1 }}>
                        ...
                        >
                          {entry.questsCompleted} quest
                          {entry.questsCompleted !== 1 ? "s" : ""} ·{" "}
                          {entry.xpFromQuests} XP
                        </div>
                      </div>
                    </div>
                  );
            ...
            >
              ...
              >
                ...
                <div style={{ minWidth: 0, flex: 1 }}>
                  ...
                  >
                    {userEntry.questsCompleted} quest
                    {userEntry.questsCompleted !== 1 ? "s" : ""} ·{" "}
                    {userEntry.xpFromQuests} XP
                  </div>
                </div>
              </div>
            </div>
