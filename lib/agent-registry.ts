...
export type AgentRegistryChangeAction =
  "registered" | "updated" | "deregistered";
...
const registry: AgentRegistryState =
  globalState.__openStellarAgentRegistry__ ?? {
    agents: new Map(),
  };
...
function normalizeCapabilities(value: unknown): string[] {
  ...
  const capabilities = value.map((capability, index) =>
    normalizeString(capability, `capabilities[${index}]`),
  );
  return Array.from(new Set(capabilities));
}

function normalizeSkillVersions(
  value: unknown,
  ...
  const versions = value.map((item, index) => {
    ...
    const version =
      item.version === undefined
        ? "1.0.0"
        : normalizeString(item.version, `skillVersions[${index}].version`);
    const minCallerVersion =
      item.minCallerVersion === undefined
        ? undefined
        : normalizeString(
            item.minCallerVersion,
...
function normalizeDependencies(value: unknown): string[] | undefined {
  ...
  const dependencies = value.map((dependency, index) =>
    normalizeString(dependency, `dependencies[${index}]`),
  );
...
function normalizeX402(value: unknown): AgentX402Manifest {
  ...
  return {
    ...
    pricePerTask:
      value.pricePerTask === undefined
        ? undefined
        : normalizeString(value.pricePerTask, "x402.pricePerTask"),
  };
}
...
function validateDependencies(agentId: string, dependencies: string[]): void {
  ...
  const reachesAgent = (currentId: string, visited: Set<string>): boolean => {
    ...
    return currentDependencies.some((dependencyId) =>
      reachesAgent(dependencyId, visited),
    );
  };

  if (
    dependencies.some((dependencyId) => reachesAgent(dependencyId, new Set()))
  ) {
...
function emitRegistryChange(
  action: AgentRegistryChangeAction,
...
export function listRegisteredAgents(
  ...
  return Array.from(registry.agents.values()).filter((agent) => {
    ...
    if (filters.skill && !agent.capabilities.includes(filters.skill))
      return false;
    if (filters.capability) {
      const needle = filters.capability.toLowerCase();
      if (!agent.capabilities.some((c) => c.toLowerCase() === needle))
        return false;
    }
    return true;
  });
...
export function registerAgent(input: unknown): AgentCapabilityManifest {
  ...
  const agent: AgentCapabilityManifest = {
    ...
    ...(normalizeSkillVersions(input.skillVersions) === undefined
      ? {}
      : { skillVersions: normalizeSkillVersions(input.skillVersions) }),
