...
type FreighterLikeResponse =
  string | { [key: string]: unknown } | null | undefined;
...
export async function connectFreighter(): Promise<{
  ...
  try {
    ...
    // Request access
    const accessResult =
      (await freighter.requestAccess()) as FreighterLikeResponse;
    ...
    if (!publicKey) {
      return {
        publicKey: null,
        error: "Freighter did not return a public key",
      };
    }
...
export async function getFreighterNetwork(): Promise<StellarNetwork | null> {
  try {
    ...
    if (
      networkDetails.networkPassphrase === STELLAR_TESTNET.networkPassphrase ||
      networkDetails.network === "TESTNET"
    ) {
      return "TESTNET";
    } else if (
      networkDetails.networkPassphrase === STELLAR_MAINNET.networkPassphrase ||
...
export async function fundTestnetAccount(
  ...
  try {
    const response = await fetch(
      `${STELLAR_TESTNET.friendbotUrl}?addr=${encodeURIComponent(publicKey)}`,
    );
...
export async function sendStellarPayment(params: {
  ...
  try {
    if (
      !validateStellarAddress(sourcePublicKey) ||
      !validateStellarAddress(destinationPublicKey)
    ) {
    ...
    const { signedXdr, error: signError } = await signTransaction(
      tx.toXDR(),
      network,
    );
...
export async function verifyStellarPayment(
  input: VerifyStellarPaymentInput,
): Promise<StellarPaymentVerification> {
  if (!/^[a-fA-F0-9]{64}$/.test(input.txHash)) {
    return {
      accepted: false,
      txHash: input.txHash,
      error: "Invalid Stellar tx hash format",
    };
  }
  ...
  try {
    ...
    if (input.memo && transaction.memo !== input.memo) {
      return {
        accepted: false,
        txHash: input.txHash,
        error: "Stellar transaction memo mismatch",
      };
    }
    ...
    if (!payment) {
      return {
        accepted: false,
        txHash: input.txHash,
        error: "Matching Stellar payment operation not found",
      };
    }
    ...
    return {
      ...
      txHash: input.txHash,
      error:
        err instanceof Error ? err.message : "Failed to verify Stellar payment",
    };
  }
}
