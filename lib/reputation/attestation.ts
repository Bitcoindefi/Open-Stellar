...
export function deriveAttestationHash(
  agentId: string,
  score: number,
  timestamp: number,
): string {
  const first = hash(
    Buffer.from(buildAttestationPayload(agentId, score, timestamp)),
  );
  const second = hash(
    Buffer.concat([Buffer.from("open-stellar-reputation"), first]),
  );
  return bytesToHex(Buffer.concat([first, second]));
}

export function createLocalReputationAttestation(
  snapshot: ReputationSnapshot,
  contractId?: string,
): ReputationAttestation {
  const timestamp = Math.floor(new Date(snapshot.updatedAt).getTime() / 1000);
  const attestationHash = deriveAttestationHash(
    snapshot.actorId,
    snapshot.score,
    timestamp,
  );
...
export function verifyLocalReputationAttestation(
  ...
  return (
    deriveAttestationHash(agentId, attestation.score, attestation.timestamp) ===
    attestation.hash
  );
}
...
export function checkReputationGate(
  ...
  if (!attestation)
    return { ok: false, error: "Reputation too low for this service" };
  return verifyLocalReputationAttestation(
    attestation.agentId,
    requirement.minReputation,
    attestation,
  )
    ? { ok: true }
