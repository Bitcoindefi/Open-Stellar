    ...
    badges: [
      {
        id: "rare-builder",
        rarity: "rare",
        awardedAt: "2026-06-26T00:00:00.000Z",
      },
    ],
    infractions: 0,
  },
...
describe("reputation attestation", () => {
  it("creates a deterministic 64-byte hash and verifies score gates", () => {
    ...
    expect(verifyLocalReputationAttestation("bot-7", 200, attestation)).toBe(
      true,
    );
    expect(verifyLocalReputationAttestation("bot-7", 500, attestation)).toBe(
      false,
    );
    expect(verifyLocalReputationAttestation("bot-8", 200, attestation)).toBe(
      false,
    );
  });
});
