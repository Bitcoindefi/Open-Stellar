...
export type ReputationBadgeRarity =
...
function normaliseMetrics(
  ...
  return {
    tasksCompleted: Math.max(
      0,
      Math.floor(Number(metrics?.tasksCompleted ?? 0)),
    ),
    x402RevenueXlm: Math.max(0, Number(metrics?.x402RevenueXlm ?? 0)),
    uptimeDaysWithoutErrors: Math.max(
      0,
      Math.floor(Number(metrics?.uptimeDaysWithoutErrors ?? 0)),
    ),
...
export function calculateReputationScore(
  ...
  const badgePoints = safe.badges.reduce(
    (sum, badge) => sum + BADGE_POINTS[badge.rarity],
    0,
  );
  ...
  return Math.max(
    ...
    Math.min(
      ...
      taskPoints + revenuePoints + uptimePoints + badgePoints - penaltyPoints,
    ),
  );
}
...
function hydrateDb(): ReputationDb {
  if (globalDb.__openStellarReputationDb__)
    return globalDb.__openStellarReputationDb__;
  const hydrated = new Map<string, ReputationSnapshot>(
    Object.entries(globalDb.__openStellarReputationStorage__ ?? {}),
  );
...
function metricsWithCurrentUptime(
  actorId: string,
  metrics: Partial<ReputationMetrics>,
): ReputationMetrics {
...
export function upsertReputationMetrics(
  actorId: string,
  metrics: Partial<ReputationMetrics>,
): ReputationSnapshot {
  const cleanId = normalizeActorId(actorId);
...
export function applyReputationAction(
  ...
  if (
    action.reason.includes("task") ||
    action.reason === "manual-update" ||
    action.reason === "perfect" ||
    action.reason === "good-service" ||
    action.reason === "voted"
  ) {
...
export function awardBadgeToAgent(
  actorId: string,
  badgeId: string,
  rarity: ReputationBadgeRarity = "common",
): ReputationSnapshot {
  const cleanId = normalizeActorId(actorId);
  const current = getReputation(cleanId);
  const metrics = { ...current.metrics };
