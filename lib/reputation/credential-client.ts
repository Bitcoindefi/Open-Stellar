...
export type ReputationCredentialSubjectSource =
  "city-agent" | "registered-agent" | "reputation-only";
...
function hydrateCredentialDb(): CredentialDb {
  ...
  const hydrated = new Map<string, ReputationCredential>(
    Object.entries(
      globalCredentials.__openStellarReputationCredentialStorage__ ?? {},
    ),
  );
...
function successRateFromReputation(
  snapshot: ReputationSnapshot | undefined,
): number {
  if (!snapshot || snapshot.metrics.tasksCompleted <= 0) return 0;
  const failures = Math.min(
    snapshot.metrics.tasksCompleted,
    snapshot.metrics.infractions,
  );
  return clampPercent(
    ((snapshot.metrics.tasksCompleted - failures) /
      snapshot.metrics.tasksCompleted) *
      100,
  );
}
...
function linksFor(
  ...
  explorerUrl: string | null,
  baseUrl?: string,
): ReputationCredential["links"] {
  return {
    shareUrl: linkFor(`/credential/${encodeURIComponent(agentId)}`, baseUrl),
    jsonUrl: linkFor(
      `/api/agents/${encodeURIComponent(agentId)}/credential/latest`,
      baseUrl,
    ),
    explorerUrl,
  };
...
export function issueReputationCredential(
  ...
  const attestation = createLocalReputationAttestation(
    reputation,
    options.contractId,
  );
  ...
  const credential: ReputationCredential = {
    ...
    proof: {
      type: isOnChain
        ? "SorobanReputationAttestation"
        : "LocalReputationAttestation",
      mode: "local-attestation",
...
export function getLatestReputationCredential(
  agentLookup: string,
): ReputationCredential | null {
  const subject = resolveReputationCredentialSubject(agentLookup);
  return (
    credentialDb.get(storageKey(subject.id)) ??
    credentialDb.get(storageKey(agentLookup)) ??
    null
  );
}
...
export function getOrIssueLatestReputationCredential(
  ...
  return credential
    ? withRequestLinks(credential, options.baseUrl)
    : issueReputationCredential(agentLookup, options);
}

export function verifyReputationCredential(
  credential: ReputationCredential,
): ReputationCredentialVerification {
  const attestation = credential.proof.attestation;
  const agentMatchesAttestation = attestation.agentId === credential.agent.id;
  const scoreMatchesAttestation =
    attestation.score === credential.reputation.score;
  const attestationHashValid = verifyLocalReputationAttestation(
    credential.agent.id,
    credential.reputation.score,
    attestation,
  );
  const proofHashMatchesAttestation =
    credential.proof.attestationHash === attestation.hash;
  const ok =
