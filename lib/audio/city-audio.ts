...
const DISTRICT_TRACKS: Record<DistrictId, { day: string; night: string }> = {
  "data-center": {
    day: "/audio/districts/data-center-day.wav",
    night: "/audio/districts/data-center-night.wav",
  },
  "comm-hub": {
    day: "/audio/districts/comm-hub-day.wav",
    night: "/audio/districts/comm-hub-night.wav",
  },
  processing: {
    day: "/audio/districts/processing-day.wav",
    night: "/audio/districts/processing-night.wav",
  },
  defense: {
    day: "/audio/districts/defense-day.wav",
    night: "/audio/districts/defense-night.wav",
  },
  research: {
    day: "/audio/districts/research-day.wav",
    night: "/audio/districts/research-night.wav",
  },
};
...
export class CityAudioEngine {
  ...
  private async setupDistrict(id: DistrictId): Promise<void> {
    ...
    try {
      const [dayBuffer, nightBuffer] = await Promise.all([
        this.loadBuffer(paths.day),
        this.loadBuffer(paths.night),
      ]);
  ...
  private applyDayNightMix(immediate: boolean): void {
    ...
    for (const node of this.districts.values()) {
      ...
      node.nightGain.gain.linearRampToValueAtTime(
        this.night ? 1 : 0,
        now + ramp,
      );
    }
