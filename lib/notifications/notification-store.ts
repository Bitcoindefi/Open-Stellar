...
const store: NotificationStore =
  globalNotifications.__notificationStore__ ?? new Map();
...
function nextCursor(): string {
  globalNotifications.__notificationCursor__ =
    (globalNotifications.__notificationCursor__ ?? 0) + 1;
  return String(globalNotifications.__notificationCursor__);
}

function clampLimit(
  limit: number | undefined,
  fallback = DEFAULT_NOTIFICATION_LIMIT,
): number {
  if (!Number.isFinite(limit)) return fallback;
  return Math.max(
    1,
    Math.min(MAX_NOTIFICATIONS_PER_AGENT, Math.floor(Number(limit))),
  );
}
...
export function listUnseenNotifications(
  ...
  return (store.get(agentId.trim()) ?? [])
    ...
    .filter(
      (notification) => !hasSince || Number(notification.cursor) > sinceCursor,
    )
    .slice()
    .reverse()
    .slice(0, limit);
...
export function getUnreadNotificationCount(agentId: string): number {
  return (store.get(agentId.trim()) ?? []).filter(
