...
const db: AgentUptimeDb =
  globalUptime.__openStellarAgentUptimeDb__ ?? new Map();
...
export function recordAgentUptimeHeartbeat(
  agentId: string,
  nowMs = Date.now(),
): AgentUptimeSnapshot {
...
export function getAgentUptime(
  agentId: string,
  nowMs = Date.now(),
): AgentUptimeSnapshot | null {
