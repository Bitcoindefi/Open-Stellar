...
const db: AgentHealthDb =
  globalHealth.__openStellarAgentHealthDb__ ?? new Map();
if (!globalHealth.__openStellarAgentHealthDb__) {
  globalHealth.__openStellarAgentHealthDb__ = db;
}

const events: AgentHealthEvent[] =
  globalHealth.__openStellarAgentHealthEvents__ ?? [];
...
function toSnapshot(
  ...
  const missedHeartbeats = Math.max(
    0,
    Math.floor(ageMs / HEARTBEAT_INTERVAL_MS),
  );
  const inferredOfflineSinceMs =
    record.offlineSinceMs ?? record.lastHeartbeatMs + OFFLINE_AFTER_MS;
  const isOffline =
    record.runtimeStatus === "offline" || ageMs > OFFLINE_AFTER_MS;
  ...
  const offlineForSeconds = offlineSinceMs
    ? Math.max(0, Math.floor((nowMs - offlineSinceMs) / 1000))
    : 0;
  const uptimeSeconds = Math.max(
    0,
    Math.floor((nowMs - record.firstHeartbeatMs) / 1000),
  );
  ...
  return {
    ...
    offlineSince: offlineSinceMs
      ? new Date(offlineSinceMs).toISOString()
      : null,
    offlineForSeconds,
    lastRestartAttempt: record.lastRestartAttemptMs
      ? new Date(record.lastRestartAttemptMs).toISOString()
      : null,
...
export function recordAgentHeartbeat(
  ...
  const record: AgentHealthRecord = {
    ...
    currentTask:
      normalizeTask(input.currentTask) ?? current?.currentTask ?? null,
    autoRestart:
      typeof input.autoRestart === "boolean"
        ? input.autoRestart
        : (current?.autoRestart ?? false),
    offlineSinceMs:
      runtimeStatus === "offline" ? (current?.offlineSinceMs ?? nowMs) : null,
    restartAttempts: current?.restartAttempts ?? 0,
    lastRestartAttemptMs:
      runtimeStatus === "offline"
        ? (current?.lastRestartAttemptMs ?? null)
        : null,
  };
...
export function seedAgentHealthRecord(
  agentId: string,
  input: AgentHeartbeatInput = {},
): AgentHealthSnapshot {
