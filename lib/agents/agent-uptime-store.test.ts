...
describe("agent uptime store", () => {
  it("counts one uptime day for each UTC calendar day with a healthy heartbeat", () => {
    ...
    recordAgentUptimeHeartbeat(
      "uptime-daily",
      Date.parse("2026-06-24T23:30:00.000Z"),
    );
    recordAgentUptimeHeartbeat(
      "uptime-daily",
      Date.parse("2026-06-25T23:15:00.000Z"),
    );
  ...
  it("does not increment more than once for multiple heartbeats on the same UTC day", () => {
    ...
    recordAgentUptimeHeartbeat(
      "uptime-same-day",
      Date.parse("2026-06-24T08:00:00.000Z"),
    );
  ...
  it("returns zero uptime days after more than 24 hours without a heartbeat", () => {
    ...
    expect(
      getAgentUptime("uptime-gap", base + 2 * 24 * 60 * 60 * 1000)?.uptimeDays,
    ).toBe(3);
    expect(
      getAgentUptime(
        "uptime-gap",
        base + 2 * 24 * 60 * 60 * 1000 + 25 * 60 * 60 * 1000,
      )?.uptimeDays,
    ).toBe(0);
  });
  ...
  it("lists current uptime snapshots sorted by agent id", () => {
    ...
    expect(listAgentUptimes(nowMs).map((uptime) => uptime.agentId)).toEqual([
      "uptime-a",
      "uptime-z",
    ]);
  });
