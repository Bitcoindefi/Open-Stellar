import { getAgentDependencies, getAgentDependents } from "@/lib/agent-registry";
...
export type AgentGraphQuery =
  { flat: boolean; maxDepth: number } | { error: "max_depth_exceeded" };
...
function summarize(nodes: InternalTreeNode[]): {
  totalCount: number;
  maxDepth: number;
} {
  let totalCount = 0;
...
export function buildAgentDependencyTree(
  agentId: string,
  maxDepth: number,
): AgentDependencyTree {
...
export function buildAgentDependentTree(
  agentId: string,
  maxDepth: number,
): AgentDependentTree {
  const nodes = buildNodes(
