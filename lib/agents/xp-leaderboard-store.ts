...
export function invalidateCacheForWindow(window: LeaderboardWindow): void {
  const cache = getCache();
...
export function recordXpEvent(
  ...
  const safeMs = Number.isFinite(timestampMs)
    ? Number(timestampMs)
    : Date.now();
...
export function getLeaderboardByAgentId(
  agentId: string,
  window: LeaderboardWindow = "all",
  nowMs: number = Date.now(),
...
export function seedXpEvents(
  events: Array<{ agentId: string; xpDelta: number; timestampMs?: number }>,
): void {
