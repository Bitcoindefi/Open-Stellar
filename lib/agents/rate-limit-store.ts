...
const store =
  globalState.__openStellarAgentRateLimitStore__ ??
  new Map<string, AgentRateLimitState>();
...
export function consumeRateLimit(
  ...
  if (state.timestamps.length >= RATE_LIMIT.maxRequests) {
    ...
    const retryAfterMs = Math.max(
      1,
      oldestRequestMs + RATE_LIMIT.windowMs - nowMs,
    );
...
export function getRateLimitStatus(
  ...
  const oldestRequestMs = state.timestamps[0];
  const resetsAtMs = oldestRequestMs
    ? oldestRequestMs + RATE_LIMIT.windowMs
    : nowMs;
