...
import {
  ...
  type DisputeResolvedEmailProps,
} from "@/emails/dispute-resolved";
import {
  WeeklySummaryEmail,
...
const globalState = globalThis as typeof globalThis & {
  __openStellarEmailState__?: EmailState;
};
const emailState: EmailState = globalState.__openStellarEmailState__ ?? {
  preferences: new Map(),
};
...
function BadgeUnlockedEmail({
  agentName,
  badgeName,
  badgeRarity,
  unsubscribeUrl,
}: BadgeUnlockedEmailInput) {
...
export async function paymentReceipt({
  to,
  data,
}: SendEmailOptions<PaymentReceiptEmailProps>) {
  return sendReactEmail(
    `Receipt: ${data.amountXlm} XLM payment to ${data.agentName}`,
    to,
    React.createElement(PaymentReceiptEmail, data),
  );
}

export async function agentLevelUp({
  to,
  data,
}: SendEmailOptions<LevelUpEmailProps>) {
  return sendReactEmail(
    `🎉 ${data.agentName} reached Level ${data.level}!`,
    to,
    React.createElement(LevelUpEmail, data),
  );
}

export async function badgeUnlocked({
  to,
  data,
}: SendEmailOptions<BadgeUnlockedEmailInput>) {
  return sendReactEmail(
    `${data.agentName} unlocked ${data.badgeName}`,
    to,
    React.createElement(BadgeUnlockedEmail, data),
  );
}

export async function disputeResolved({
  to,
  data,
}: SendEmailOptions<DisputeResolvedEmailProps>) {
  return sendReactEmail(
    `Dispute ${data.escrowId} resolved`,
    to,
    React.createElement(DisputeResolvedEmail, data),
  );
}
...
export async function weeklyReport({
  to,
  data,
}: SendEmailOptions<WeeklySummaryEmailProps>) {
  return sendReactEmail(
    `Your Open Stellar week: ${data.totalTasks.toLocaleString()} tasks, ${data.totalXlmEarned} XLM earned`,
    to,
    React.createElement(WeeklySummaryEmail, data),
  );
}
