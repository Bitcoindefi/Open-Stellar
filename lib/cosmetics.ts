...
export const SKINS: SkinDef[] = [
  {
    ...
    name: "Default",
    levelRequired: 1,
    description: "Standard robot sprite",
  },
  {
    id: "neon",
    name: "Neon",
    levelRequired: 10,
    description: "Color outline matching district color",
  },
  {
    id: "chrome",
    name: "Chrome",
    levelRequired: 20,
    description: "Metallic sheen overlay",
  },
  {
    id: "hologram",
    name: "Hologram",
    levelRequired: 30,
    description: "Semi-transparent with scanline effect",
  },
  {
    id: "gold",
    name: "Gold",
    levelRequired: 40,
    description: "Gold tint + sparkle frame",
  },
  {
    id: "legendary",
    name: "Legendary",
    levelRequired: 50,
    description: "Animated aura + particle trail",
  },
];
...
export function getUnlockedBadgeIds(
  agent: MoltbotAgent,
  allAgents: MoltbotAgent[],
): string[] {
  return BADGES.filter((badge) => badge.isUnlocked(agent, allAgents)).map(
    (badge) => badge.id,
  );
}
...
export function getUnlockedSkinIds(agent: MoltbotAgent): SkinId[] {
  ...
  return SKINS.filter((skin) => level >= skin.levelRequired).map(
    (skin) => skin.id,
  );
}
...
export function getUnlockedAccessoryIds(
  ...
  return ACCESSORIES.filter((accessory) => badgeIds.has(accessory.badgeId)).map(
...
export function isAccessoryUnlockedForBadges(
