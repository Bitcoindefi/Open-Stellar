...
export async function verifyEvmPayment(
  ...
  if (!isHexTxHash(input.txHash)) {
    return {
      ...
      txHash: input.txHash,
      confirmations: 0,
      error: "Invalid EVM tx hash format",
    };
  }
  ...
  try {
    ...
    return {
      ...
      txHash: input.txHash,
      confirmations: 0,
      error: "Invalid expected EVM recipient address",
    };
  }
  ...
  const confirmations = receipt.blockNumber
    ? Number(currentBlock - receipt.blockNumber + BigInt(1))
    : 0;
  ...
  if (confirmations < minConfirmations) {
    return {
      ...
      txHash: input.txHash,
      confirmations,
      error: "Insufficient EVM confirmations",
    };
  }
  ...
  if (!addressesEqual(tx.to, expectedTo)) {
    return {
      ...
      to: tx.to ?? undefined,
      valueWei,
      error: "EVM transaction recipient mismatch",
    };
  }
  ...
  if (input.expectedFrom && !addressesEqual(tx.from, input.expectedFrom)) {
    return {
      ...
      to: tx.to ?? undefined,
      valueWei,
      error: "EVM transaction sender mismatch",
    };
  }
  ...
  if (tx.value < expectedValueWei) {
    return {
      ...
      to: tx.to ?? undefined,
      valueWei,
      error: `EVM payment too small: ${formatEther(tx.value)} native`,
    };
  }
