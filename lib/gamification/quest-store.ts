...
export function listStoredQuests(options?: {
...
function getQuestProgressParticipants(quest: StoredQuest): Array<{
  ...
  const progressByAgent = new Map<
    string,
    { completedSubtasks: number; hasProgress: boolean }
  >();
  ...
  for (const subTask of subTasks) {
    ...
    const current = progressByAgent.get(agentId) ?? {
      completedSubtasks: 0,
      hasProgress: false,
    };
...
export function setQuestExpired(id: string): {
  quest: StoredQuest | null;
  notified: string[];
} {
  const existingQuest = getStoredQuest(id);
  const participants = existingQuest
    ? getQuestProgressParticipants(existingQuest)
    : [];
...
export function seedQuest(
  quest: Partial<StoredQuest> & { id: string },
): StoredQuest {
  const store = getStore();
