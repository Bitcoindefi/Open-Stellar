...
import {
  ...
  getAgentDistrictCompetitionScore,
  getWeekBounds,
...
const SNAPSHOT_PATH = path.join(
  process.cwd(),
  "data",
  "district-leaderboard-snapshots.json",
);

export function getDistrictLeaderboard(
  agents: MoltbotAgent[],
  districtId: DistrictId,
  now: Date = new Date(),
  ...
  return agents
    ...
    .map((agent) => {
      const score = getAgentDistrictCompetitionScore(
        agent,
        event.challenge,
        event.weekIndex,
      );
...
export function getPreviousDistrictLeaderboardSnapshot(
  districtId: DistrictId,
  now: Date = new Date(),
): DistrictLeaderboardSnapshot | null {
  const previousWeekIndex = Math.max(0, getWeekBounds(now).weekIndex - 1);
  return (
    readDistrictLeaderboardSnapshots()
      .filter(
        (snapshot) =>
          snapshot.districtId === districtId &&
