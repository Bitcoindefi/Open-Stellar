...
export function getSkillUpgradeCost(
  skill: Pick<Skill, "level" | "maxLevel">,
): number | null {
...
export function getSkillUpgradeState(skill: Skill): SkillUpgradeState {
  ...
  const progressPct = isMaxLevel
    ? 100
    : Math.min(100, Math.round((skill.xp / cost) * 100));
...
export function upgradeSkill(skill: Skill): SkillUpgradeResult {
  ...
  const nextSkill = {
    ...
    xpToNext:
      getSkillUpgradeCost({ level: nextLevel, maxLevel: skill.maxLevel }) ?? 0,
  };
...
export function upgradeAgentSkill(
  agent: MoltbotAgent,
  skillId: string,
): { agent: MoltbotAgent; result: SkillUpgradeResult | null } {
