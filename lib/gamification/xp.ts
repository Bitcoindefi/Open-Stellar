...
export function getXpToNextLevel(level: number): number {
  ...
  for (let currentLevel = 1; currentLevel <= level; currentLevel += 1) {
    total += Math.round(
      FIRST_LEVEL_XP_THRESHOLD *
        Math.pow(LEVEL_XP_MULTIPLIER, Math.max(0, currentLevel - 1)),
    );
  }
  return total;
}
...
export function checkLevelUp(
  xp: number,
  currentLevel = 1,
): { level: number; xpToNext: number; leveledUp: boolean } {
...
export function awardTaskXP(input: TaskXpInput): XPAwardResult[] {
  const awards = [
    awardXP(input.agentId, XP_AWARDS.TASK_COMPLETED, "task.completed"),
  ];
  if (
    typeof input.durationMs === "number" &&
    input.durationMs <= FAST_TASK_MAX_DURATION_MS
  ) {
    awards.push(
      awardXP(input.agentId, XP_AWARDS.FAST_TASK_BONUS, "task.completed.fast"),
    );
  }
  return awards;
}
...
export function awardSkillXP(
  ...
  return skills.map((skill) => {
    ...
    return {
      ...
      xp,
      xpToNext:
        getSkillUpgradeCost({ level: skill.level, maxLevel: skill.maxLevel }) ??
        0,
    };
  });
}

export function applyXPAwardToAgent(
  agent: MoltbotAgent,
  award: Pick<XPAwardResult, "xp" | "level">,
): MoltbotAgent {
  return {
    ...agent,
    xp: award.xp,
