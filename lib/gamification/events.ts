...
export type DistrictChallengeType =
  "throughput" | "revenue" | "uptime" | "speed" | "skill";
...
export const DISTRICT_CHALLENGES: DistrictChallengeDefinition[] = [
  {
    weekMod: 0,
    type: "throughput",
    name: "Throughput Race",
    metric: "Most tasks completed",
    scoreLabel: "tasks",
    better: "higher",
  },
  {
  ...
  },
  {
    ...
    metric: "Highest aggregate uptime %",
    scoreLabel: "uptime",
    better: "higher",
  },
  {
    ...
    type: "speed",
    name: "Speed Trial",
    metric: "Lowest average task duration",
    scoreLabel: "avg duration",
    better: "lower",
  },
  {
    weekMod: 4,
    type: "skill",
    name: "Skill Mastery",
    metric: "Highest average skill level",
    scoreLabel: "skill avg",
    better: "higher",
  },
];
...
export function getWeekBounds(now: Date = new Date()): {
  ...
  return {
    weekIndex,
    startedAt,
    endsAt: new Date(startedAt.getTime() + WEEK_MS),
  };
}
...
export function formatDistrictScore(
  challenge: DistrictChallengeDefinition,
  score: number,
): string {
...
export function getAgentDistrictCompetitionScore(
  agent: MoltbotAgent,
  challenge: DistrictChallengeDefinition,
  weekIndex: number,
): number {
  ...
  switch (challenge.type) {
    ...
    case "skill":
      return agent.skills.length === 0
        ? 0
        : agent.skills.reduce((sum, skill) => sum + skill.level, 0) /
            agent.skills.length;
  }
}

export function getWeeklyTasksCompleted(
  agent: MoltbotAgent,
  weekIndex: number,
): number {
  const completionRatio =
    0.12 + seededNoise(`${weekIndex}:weekly-tasks:${agent.id}`) * 0.28;
  return Math.max(0, Math.round(agent.tasksCompleted * completionRatio));
}

export function getActiveDistrictEvent(
  now: Date = new Date(),
): ActiveDistrictEvent {
  const bounds = getWeekBounds(now);
  const challenge =
    DISTRICT_CHALLENGES[bounds.weekIndex % DISTRICT_CHALLENGES.length];
  return {
    ...
    secondsRemaining: Math.max(
      0,
      Math.floor((bounds.endsAt.getTime() - now.getTime()) / 1000),
    ),
    rewards: [
      {
        districtId: "data-center",
        districtName: "Winning district",
        multiplier: 2,
        durationHours: 24,
        description: "2× XP multiplier for all agents",
      },
      {
        districtId: "comm-hub",
        districtName: "Second district",
        multiplier: 1.5,
        durationHours: 24,
        description: "1.5× XP multiplier for all agents",
      },
    ],
  };
}

export function getDistrictStandings(
  agents: MoltbotAgent[],
  now: Date = new Date(),
): DistrictStanding[] {
  const event = getActiveDistrictEvent(now);
  const standings = DISTRICTS.map((district) => {
    const districtAgents = agents.filter(
      (agent) => agent.district === district.id,
    );
    const scoredAgents = districtAgents.map((agent) => ({
      agent,
      score: getAgentDistrictCompetitionScore(
        agent,
        event.challenge,
        event.weekIndex,
      ),
    ...
    const score =
      event.challenge.type === "speed"
        ? scoredAgents.reduce((sum, entry) => sum + entry.score, 0) /
          Math.max(1, scoredAgents.length)
        : scoredAgents.reduce((sum, entry) => sum + entry.score, 0) /
          (event.challenge.type === "uptime" || event.challenge.type === "skill"
            ? Math.max(1, scoredAgents.length)
            : 1);
    const top = [...scoredAgents].sort((a, b) =>
      event.challenge.better === "lower"
        ? a.score - b.score
        : b.score - a.score,
    )[0];
    ...
    event.challenge.better === "lower" ? a.score - b.score : b.score - a.score,
  );
...
export function getDistrictEventHistory(
  agents: MoltbotAgent[],
  now: Date = new Date(),
  count = 6,
): DistrictEventHistoryEntry[] {
  const current = getWeekBounds(now).weekIndex;
  return Array.from({ length: count }, (_, offset) => {
    const weekIndex = Math.max(0, current - offset - 1);
    const eventDate = new Date(
      EPOCH_START_MS + weekIndex * WEEK_MS + WEEK_MS - DAY_MS,
    );
    const standings = getDistrictStandings(agents, eventDate);
    const winner = standings[0];
    const challenge =
      DISTRICT_CHALLENGES[weekIndex % DISTRICT_CHALLENGES.length];
    return {
      ...
      endedAt: new Date(
        EPOCH_START_MS + (weekIndex + 1) * WEEK_MS,
      ).toISOString(),
    };
  });
}
