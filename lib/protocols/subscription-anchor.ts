...
const proofRegistry: SubscriptionProofRegistry =
  globalState.__x402SubscriptionProofRegistry__ ?? new Map();
...
function currentSubscriptionActive(subscriptionId: string): boolean {
  ...
  return checkX402Subscription(subscription.agentId, subscription.serviceId)
    .active;
}
...
export function getSubscriptionPaymentProof(
  subscriptionId: string,
): SubscriptionPaymentProof | undefined {
