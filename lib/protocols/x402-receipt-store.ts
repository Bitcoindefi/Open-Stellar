...
export function saveX402Receipt(
  receipt: X402ExplorerReceipt,
): X402ExplorerReceipt {
...
export function getX402Receipt(
  receiptId: string,
): X402ExplorerReceipt | undefined {
  return readReceipts().find((receipt) => receipt.id === receiptId);
}

export function listX402Receipts(
  filters: X402ReceiptQuery = {},
): X402ReceiptPage {
  const pageSize = Math.max(
    1,
    Math.min(50, Math.floor(filters.pageSize ?? 50)),
  );
  ...
  const filtered = allReceipts.filter((receipt) => {
    ...
    if (
      agent &&
      receipt.agentId.toLowerCase() !== agent &&
      receipt.agent.toLowerCase() !== agent
    )
      return false;
    if (
      service &&
      receipt.serviceId.toLowerCase() !== service &&
      receipt.service.toLowerCase() !== service
    )
      return false;
    if (q) {
      const haystack = [
