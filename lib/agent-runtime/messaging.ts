...
export type AgentMessageType =
  "task.request" | "task.response" | "data.share" | "status.query" | "chat";
...
const messageBus: MessageBusState =
  globalState.__openStellarAgentMessageBus__ ?? {
    messages: [],
    listeners: new Map(),
    sequence: 0,
  };
...
export function broadcastDistrictMessage(
  districtId: DistrictId,
  input: Omit<AgentMessageInput, "toAgentId" | "districtId">,
): AgentMessage {
...
export function listAgentMessages(agentId: string): AgentMessage[] {
  return messageBus.messages.filter(
    (message) =>
...
export function listDistrictMessages(districtId: DistrictId): AgentMessage[] {
  return messageBus.messages.filter(
    (message) => message.districtId === districtId,
  );
}
...
export function subscribeToAgentMessages(
  ...
  const listeners =
    messageBus.listeners.get(agentId) ?? new Set<MessageListener>();
...
export function createAgentMessageStream(agentId: string) {
  ...
  return new ReadableStream<Uint8Array>({
    start(controller) {
      controller.enqueue(
        encoder.encode(`: open-stellar messages for ${agentId}\n\n`),
      );
