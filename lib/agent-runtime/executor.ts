...
import {
  recordClaudeTaskCost,
  type ClaudeCostRecord,
} from "@/lib/agent-runtime/costs";
...
export function formatTask(task: AgentTask): string {
  return JSON.stringify(
    { id: task.id, title: task.title, payload: task.payload },
    null,
    2,
  );
}
  ...
  return {
    messages: {
      async create(input) {
        ...
        if (!response.ok)
          throw new Error(
            `Claude API failed with ${response.status}: ${await response.text()}`,
          );
...
export async function executeTask(
  ...
  const client =
    options.client ??
    createFetchClaudeClient(
      options.apiKey ?? process.env.ANTHROPIC_API_KEY ?? "",
    );
  if (!options.client && !(options.apiKey ?? process.env.ANTHROPIC_API_KEY))
    throw new Error(
      "ANTHROPIC_API_KEY is required to execute Claude-backed agent tasks",
    );
