...
describe("Agent SDK lifecycle", () => {
  ...
  it("restart() calls stop() then start()", async () => {
    ...
    const calls: string[] = [];
    const stopSpy = vi.spyOn(agent, "stop").mockImplementation(async () => {
      calls.push("stop");
    });
    const startSpy = vi.spyOn(agent, "start").mockImplementation(async () => {
      calls.push("start");
    });
  ...
  it("executeTask() on a stopped agent throws an error", async () => {
    ...
    await expect(agent.executeTask(createTask())).rejects.toThrow(
      "Cannot execute task on a stopped agent",
    );
  });

  it("heartbeat updates lastSeen timestamp every interval", async () => {
    const agent = new Agent(
      createConfig("agent-heartbeat", { heartbeatIntervalMs: 2_500 }),
    );
    await agent.start();
