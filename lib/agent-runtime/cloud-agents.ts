...
const globalCloud = globalThis as typeof globalThis & {
  __openStellarCloudAgents__?: Map<string, CloudAgentConfig>;
};
const configs =
  globalCloud.__openStellarCloudAgents__ ?? new Map<string, CloudAgentConfig>();
if (!globalCloud.__openStellarCloudAgents__)
  globalCloud.__openStellarCloudAgents__ = configs;
...
export function provisionCloudAgent(
  input: {
    name?: string;
    model?: string;
    district?: DistrictId;
  ...
  const district = DISTRICTS.some((d) => d.id === input.district)
    ? input.district!
    : "research";
  ...
  recordAgentHeartbeat(id, {
    status: "active",
    cpu: 5,
    memory: 18,
    currentTask: "Waiting for cloud tasks",
    autoRestart: true,
  });
...
export function listCloudAgentConfigs(): CloudAgentConfig[] {
  return Array.from(configs.values()).sort((a, b) =>
...
export function updateCloudAgentResult(
  id: string,
  summary: string,
  ...
  const updated = {
    ...
    lastResult: summary.slice(0, 240),
  };
  configs.set(id, updated);
  return updated;
}

export function cloudConfigToAgent(
  config: CloudAgentConfig,
  index = 0,
): MoltbotAgent {
  const district =
    DISTRICTS.find((d) => d.id === config.district) ?? DISTRICTS[0];
  ...
  return {
    ...
    currentTask: health.degraded
      ? `Degraded: ${health.errorCount24h} errors in 24h`
      : (config.lastResult ?? "Waiting for cloud tasks"),
    taskProgress: 0,
    color: "#38bdf8",
    pixelX: x,
    pixelY: y,
