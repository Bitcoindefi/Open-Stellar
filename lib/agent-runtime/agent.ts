...
const globalState = globalThis as typeof globalThis & {
  __openStellarAgentRuntime__?: RuntimeState;
};
const runtimeState: RuntimeState =
  (globalState.__openStellarAgentRuntime__ ??= {
    agents: new Map(),
    tasks: new Map(),
  });
...
function normalizeTask(input: unknown): Task {
  ...
  return {
    ...
    createdAt: typeof body.createdAt === "string" ? body.createdAt : isoNow(),
  };
}
...
export function getAgentTaskRecord(
  agentId: string,
  taskId: string,
): TaskRecord | null {
  return (
    listAgentTaskRecords(agentId).find((record) => record.task.id === taskId) ??
    null
  );
}
...
export class Agent implements AgentRuntimeContext {
  ...
  private startHooks: (() => void | Promise<void>)[] = [];
  private stopHooks: (() => void | Promise<void>)[] = [];
  private errorHooks: ((error: Error) => void | Promise<void>)[] = [];
  private stateChangeHooks: ((status: AgentStatus) => void | Promise<void>)[] =
    [];
  ...
  onStateChange(handler: (status: AgentStatus) => void | Promise<void>): void {
    this.stateChangeHooks.push(handler);
  }
  ...
  async executeTask(taskInput: Task): Promise<TaskResult> {
    if (this.status === "stopped" || this.status === "offline")
      throw new Error("Cannot execute task on a stopped agent");
    ...
    publishSystemEvent({
      type: "task.started",
      agentId: this.id,
      task: { id: task.id, title: task.title, district: task.district },
    });
    ...
    try {
      ...
      const result: TaskResult = {
        ...
        summary:
          typeof handled === "string"
            ? handled
            : (handled?.summary ?? `Completed task: ${task.title}`),
      ...
      publishSystemEvent({
        ...
        agentId: this.id,
        taskId: task.id,
        result: { summary: result.summary, durationMs },
      });
      return result;
    } catch (error) {
      ...
      const health = recordAgentExecutionError({
        agentId: this.id,
        error,
        taskExcerpt: task.title,
      });
      this.setStatus(health.degraded ? "degraded" : "error");
      this.recordHeartbeat(task.title);
      writeTaskRecord(this.id, {
        task,
        result,
      ...
      publishSystemEvent({
        ...
        agentId: this.id,
        taskId: task.id,
        result: { summary: result.error ?? result.summary, durationMs },
      });
      return result;
    }
  }
  ...
  getStatus(): AgentStatus {
    if (
      ...
      Date.now() - this.lastHeartbeatMs >
        (this.config.offlineAfterMs ?? DEFAULT_OFFLINE_AFTER_MS)
    )
      return "offline";
    return this.status;
  }
  ...
  getMetrics(): AgentMetrics {
    const uptimeMs = this.startedAtMs
      ? Math.max(0, (this.stoppedAtMs ?? Date.now()) - this.startedAtMs)
      : 0;
    const totalDuration = this.taskDurations.reduce(
    ...
    return {
      ...
      startedAt: this.startedAtMs
        ...
        : null,
      stoppedAt: this.stoppedAtMs
        ? new Date(this.stoppedAtMs).toISOString()
        : null,
      lastHeartbeat: this.lastHeartbeatMs
        ? new Date(this.lastHeartbeatMs).toISOString()
        : null,
      uptimeMs,
      averageTaskDurationMs: this.taskDurations.length
        ? Math.round(totalDuration / this.taskDurations.length)
        : 0,
    };
  }
