...
const runtimeState: RuntimeState = globalState.__openStellarAgentRuntime__ ?? {
  agents: new Map(),
  tasks: new Map(),
};
if (!globalState.__openStellarAgentRuntime__)
  globalState.__openStellarAgentRuntime__ = runtimeState;
...
function normalizeTask(input: unknown): Task {
  ...
  return {
    id: String(
      body.id || `task_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`,
    ),
