...
function priceForModel(model: string) {
  const key = Object.keys(MODEL_PRICES_PER_MILLION).find((candidate) =>
    model.includes(candidate),
  );
...
export function recordClaudeTaskCost(
  input: Omit<ClaudeCostRecord, "id" | "createdAt" | "costUsd">,
): ClaudeCostRecord {
...
export function getAgentClaudeAnalytics(agentId: string, dailyBudgetUsd = 5) {
  ...
  const lifetimeSpendUsd = agentRecords.reduce(
