...
const DEFAULT_DELIVERY_LOG_PATH = join(
  process.cwd(),
  ".data",
  "webhook-delivery-log.jsonl",
);
const MAX_DELIVERY_LOG_ENTRIES = 200;

let deliveryLogPath =
  process.env.WEBHOOK_DELIVERY_LOG_PATH || DEFAULT_DELIVERY_LOG_PATH;
...
function isWebhookDeliveryAttempt(
  value: unknown,
): value is WebhookDeliveryAttempt {
...
function parseDeliveryAttempt(line: string): WebhookDeliveryAttempt | null {
  try {
    ...
    if (parsed && typeof parsed === "object" && !("attempt" in parsed)) {
      ...
      return isWebhookDeliveryAttempt(withDefaultAttempt)
        ? withDefaultAttempt
        : null;
    }
...
export function appendWebhookDeliveryAttempt(
  input: CreateWebhookDeliveryAttempt,
): WebhookDeliveryAttempt {
  const attempt: WebhookDeliveryAttempt = {
    id: `wha_${randomBytes(8).toString("hex")}`,
    ...input,
  };
  const attempts = [...readDeliveryAttempts(), attempt].slice(
