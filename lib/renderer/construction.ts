...
export function drawConstruction(
  ...
  const p3 = Math.min(
    PHASE_3_MS,
    Math.max(0, elapsedMs - PHASE_1_MS - PHASE_2_MS),
  );
  ...
  for (let i = 0; i < sparksCount; i++) {
    ...
    const sx = lerp(
      district.x + 10,
      district.x + district.w - 10,
      Math.sin(seed) * 0.5 + 0.5,
    );
    const siteY = buildingsBaseY + 40 - Math.sin(seed * 0.3 + tick * 0.05) * 2;
    const fall =
      localT < sparkIntensity ? localT / Math.max(0.001, sparkIntensity) : 1;
  ...
  if (decoT > 0) {
    ...
    if (signReveal > 0) {
      ...
      const shown = name.slice(
        0,
        Math.max(0, Math.floor(name.length * signReveal)),
      );
