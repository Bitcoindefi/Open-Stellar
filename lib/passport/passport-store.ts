 ...
 */
export function registerPassportForExpiry(
  passport: AgentPassport,
): PassportExpiryRecord {
  const db = getDb();
 ...
 */
export function runPassportExpiryCheck(
  nowMs = Date.now(),
): PassportExpiryCheckResult {
  const db = getDb();
  ...
  for (const record of db.passports.values()) {
    ...
    if (expiresAtMs <= nowMs) {
      ...
      const { wasRevoked } = revokePassportInStore(
        record.passportId,
        "expired",
        nowMs,
      );
 ...
 */
export function seedPassportExpiryRecord(
  passport: AgentPassport,
): PassportExpiryRecord {
