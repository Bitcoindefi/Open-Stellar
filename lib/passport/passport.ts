...
export const PASSPORT_TTL_DAYS = Number(
  process.env.NEXT_PUBLIC_PASSPORT_TTL_DAYS ?? 90,
);
export const PASSPORT_TTL_MS = PASSPORT_TTL_DAYS * 24 * 60 * 60 * 1000;
export const PROOF_CACHE_TTL_MS = Number(
  process.env.NEXT_PUBLIC_PASSPORT_PROOF_CACHE_TTL_MS ?? 15 * 60 * 1000,
);
...
export const CONTRACTS = {
  get validator() {
    return networks[currentNetwork()].contractId;
  },
  get verifier() {
    return networks[currentNetwork()].verifierContractId;
  },
  get rpcUrl() {
    return networks[currentNetwork()].rpcUrl;
  },
  get networkPassphrase() {
    return networks[currentNetwork()].networkPassphrase;
  },
};
...
function client(publicKey = VIEWER, network = currentNetwork()) {
  return new Client({
    contractId: networks[network].contractId,
...
const collectionKey = (agentId: string, network = currentNetwork()) =>
  `open-stellar:passport-collection:${network}:${agentId}`;
export function getPassportStatus(
  passport: Pick<AgentPassport, "expiresAt" | "status">,
  now = new Date(),
): PassportStatus {
  if (passport.status === "REVOKED") return "REVOKED";
  return new Date(passport.expiresAt).getTime() <= now.getTime()
    ? "EXPIRED"
    : "ACTIVE";
}
export function toAgentPassport(
  m: MintedProof,
  txHash?: string,
): AgentPassport {
  return {
    ...
    agentId: m.agentId,
    spendCap: m.spendCap,
    registryRoot: m.registryRoot,
    nullifierHash: m.nullifierHash,
    issuedAt: m.issuedAt,
    expiresAt: m.expiresAt,
    status: getPassportStatus(m),
    network: m.network,
    txHash,
    proof: m,
  };
...
export function loadPassportCollection(agentId: string): PassportCollection {
  if (typeof localStorage === "undefined")
    return { agentId, passports: [], primaryPassport: "" };
  const parsed = JSON.parse(
    localStorage.getItem(collectionKey(agentId)) ?? "null",
  ) as PassportCollection | null;
  const passports = (parsed?.passports ?? []).map((p) => ({
    ...p,
    status: getPassportStatus(p),
  }));
  return {
    agentId,
    passports,
    primaryPassport:
      parsed?.primaryPassport ??
      passports.find((p) => p.status === "ACTIVE")?.id ??
      passports[0]?.id ??
      "",
  };
}
export function savePassport(passport: AgentPassport): PassportCollection {
  ...
  const passports = [
    passport,
    ...current.passports.filter((p) => p.id !== passport.id),
  ];
  const primaryPassport =
    passport.status === "ACTIVE" ? passport.id : current.primaryPassport;
  const collection = { agentId: passport.agentId, passports, primaryPassport };
  localStorage.setItem(
    collectionKey(passport.agentId, passport.network),
    JSON.stringify(collection),
  );
  return collection;
}
export function revokePassportLocal(
...
export async function revokePassport(
  ...
  return {
    ok: false,
    error:
      "On-chain revocation requires the upgraded validator blacklist admin method to be deployed.",
  };
}

const cacheKey = (spendCap: string, agentId = "new") =>
  `open-stellar:proof-cache:${currentNetwork()}:${agentId}:${spendCap}`;
...
async function readProofCache(key: string): Promise<MintedProof | null> {
  ...
  return new Promise((resolve) => {
    const req = db
      .transaction("proofs", "readonly")
      .objectStore("proofs")
      .get(key);
    req.onerror = () => resolve(null);
    req.onsuccess = () => {
      const parsed = req.result as
        { expiresAt: number; value: MintedProof } | undefined;
...
async function writeProofCache(key: string, value: MintedProof): Promise<void> {
  ...
  await new Promise<void>((resolve) => {
    const req = db
      ...
      .objectStore("proofs")
      .put({ expiresAt: Date.now() + PROOF_CACHE_TTL_MS, value }, key);
    req.onerror = () => resolve();
...
export async function mintPassport(
  spendCap: string,
  options: { agentId?: string; useCache?: boolean } = {},
): Promise<MintedProof> {
  ...
  const minted = {
    ...
    id: passportId(currentNetwork(), agentId, nullifierHash),
    agentId,
    spendCap,
    registryRoot,
    nullifierHash,
    issuedAt: issuedAt.toISOString(),
    expiresAt: new Date(issuedAt.getTime() + PASSPORT_TTL_MS).toISOString(),
...
export async function isRegistered(agentId: string): Promise<boolean> {
  return (await client().is_registered({ agent_id: BigInt(agentId) })).result;
}
...
export async function authorizePayment(
  ...
  const collection =
    typeof localStorage !== "undefined"
      ? loadPassportCollection(agentId)
      : undefined;
  const activeLocal =
    collection?.passports.find(
      (p) =>
        p.id === collection.primaryPassport &&
        getPassportStatus(p) === "ACTIVE",
    ) ?? collection?.passports.find((p) => getPassportStatus(p) === "ACTIVE");
  const passport = activeLocal
    ? { spend_cap: activeLocal.spendCap }
    : await getPassport(agentId);
  if (!passport)
    return {
      authorized: false,
      reason: "No active passport — agent not verified",
    };
