...
describe("agent position history pagination", () => {
  ...
  it("total always reflects the full unfiltered count", () => {
    ...
    const result1 = getAgentPositionHistoryPaginated("agent-paginate", {
      limit: 10,
    });
...
describe("GET /api/agents/[id]/positions", () => {
  ...
  it("falls back to default limit when limit > 1000", async () => {
    ...
    const req = new Request(
      "http://localhost/api/agents/agent-api/positions?limit=1001",
    );
    const res = await GET(req, {
      params: Promise.resolve({ id: "agent-api" }),
    });
  ...
  it("falls back to default limit when limit is not a positive integer", async () => {
    ...
    const req = new Request(
      "http://localhost/api/agents/agent-api/positions?limit=abc",
    );
    const res = await GET(req, {
      params: Promise.resolve({ id: "agent-api" }),
    });
  ...
  it("respects limit query param", async () => {
    ...
    const req = new Request(
      "http://localhost/api/agents/agent-api/positions?limit=5",
    );
    const res = await GET(req, {
      params: Promise.resolve({ id: "agent-api" }),
    });
  ...
  it("respects before query param", async () => {
    ...
    const req = new Request(
      "http://localhost/api/agents/agent-api/positions?before=2026-06-01T01:00:00.000Z",
    );
    const res = await GET(req, {
      params: Promise.resolve({ id: "agent-api" }),
    });
  ...
  it("respects after query param", async () => {
    ...
    const req = new Request(
      "http://localhost/api/agents/agent-api/positions?after=2026-06-01T02:00:00.000Z",
    );
    const res = await GET(req, {
      params: Promise.resolve({ id: "agent-api" }),
    });
