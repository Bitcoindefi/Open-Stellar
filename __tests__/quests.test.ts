...
describe("quests", () => {
  it("sets daily reset to the next UTC midnight", () => {
    expect(
      getNextDailyReset(new Date("2026-06-26T13:45:00.000Z")).toISOString(),
    ).toBe("2026-06-27T00:00:00.000Z");
  });

  it("sets weekly reset to Sunday midnight UTC", () => {
    expect(
      getNextWeeklyReset(new Date("2026-06-26T13:45:00.000Z")).toISOString(),
  ...
  it("builds daily, weekly, and story quests with rewards and completion state", () => {
    const quests = buildQuests(
      completeStats,
      new Date("2026-06-26T13:45:00.000Z"),
    );
    ...
    expect(
      quests.find((quest) => quest.id === "daily-complete-5-tasks")?.reward,
    ).toMatchObject({ xp: 50, xlm: "0.05" });
  });
  ...
  it("includes optional minimum reputation requirements", () => {
    ...
    expect(
      quests.find((quest) => quest.id === "weekly-onboard-marketplace-service")
        ?.minReputation,
    ).toBe(50);
    expect(
      quests.find((quest) => quest.id === "daily-complete-5-tasks")
        ?.minReputation,
    ).toBeUndefined();
  });
  ...
  it("keeps incomplete quest progress clamped between zero and one hundred", () => {
    ...
    expect(
      quests.find((quest) => quest.id === "daily-complete-5-tasks")?.progress,
    ).toBe(40);
    expect(
      quests.find((quest) => quest.id === "story-first-subscription")?.progress,
    ).toBe(0);
  });
  ...
  it("does not generate quest notifications from the read-only quests API", async () => {
    const res = await getQuestsRoute(
      new Request("http://localhost/api/quests?agentId=quest-route-agent"),
    );
