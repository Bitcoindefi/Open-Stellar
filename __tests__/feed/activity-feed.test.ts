...
describe("activity feed", () => {
  it("lists seeded feed events with filters", () => {
    ...
    expect(
      dataCenter.every((event) => event.districtId === "data-center"),
    ).toBe(true);
  });
  ...
  it("paginates seeded events with a cursor", () => {
    ...
    const secondPage = listFeedEvents({
      limit: 2,
      cursor: firstPage.at(-1)?.occurredAt,
    });
  ...
  it("serves JSON feed API responses", async () => {
    const res = await getFeed(
      new Request("http://localhost/api/feed?kind=payment&limit=1"),
    );
