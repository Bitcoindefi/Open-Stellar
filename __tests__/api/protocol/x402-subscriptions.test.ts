...
describe("x402 subscriptions", () => {
  ...
  it("creates a recurring subscription and charges the first month", async () => {
    const req = new Request(
      "http://localhost/api/protocol/x402/subscriptions",
      {
        method: "POST",
        body: JSON.stringify({
  ...
  it("checks active access and consumes calls for withX402 gates", async () => {
    ...
    const first = await checkSubscription(
      new Request(
        "http://localhost/api/protocol/x402/subscriptions/nexus-7/my-data-api?consume=true",
      ),
      {
    ...
    const second = checkX402Subscription("nexus-7", "my-data-api", {
      consumeCall: true,
    });
  ...
  it("moves renewals into grace and then paused when wallet balance is insufficient", () => {
    ...
    const grace = renewX402Subscriptions(new Date("2026-07-22T12:00:00.000Z"), {
      "nexus-7": 1,
    });
    ...
    const paused = renewX402Subscriptions(
