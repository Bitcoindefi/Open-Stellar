...
import {
  ...
  SUBSCRIPTION_ANCHOR_CONTRACT_ID,
...
describe("GET /api/subscriptions/:id/proof", () => {
  ...
  it("returns the latest anchored payment proof for a settled subscription", async () => {
    ...
    const quoteRes = await postQuote(
      new Request("http://localhost/api/protocol/x402/quote", {
        method: "POST",
        body: JSON.stringify({
    ...
    const settleRes = await postSettle(
      new Request("http://localhost/api/protocol/x402/settle", {
        method: "POST",
        body: JSON.stringify({
    ...
    const proofRes = await getSubscriptionProof(
      new Request(
        `http://localhost/api/subscriptions/${subscription.id}/proof`,
      ),
      {
