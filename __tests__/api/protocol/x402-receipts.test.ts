...
describe("GET /api/protocol/x402/receipts", () => {
  it("lists persisted receipts and filters by agent", async () => {
    ...
    const res = await listReceipts(
      new Request(
        "http://localhost/api/protocol/x402/receipts?agent=nexus-7&pageSize=50",
      ),
    );
    ...
    const targetReceipt = data.receipts.find(
      (r: any) => r.service === "data-indexing",
    );
    ...
    expect(targetReceipt.explorerUrl).toMatch(
      /stellar\.expert\/explorer\/(mainnet|testnet)\/tx\//,
    );
  });
  ...
  it("returns a single receipt by id", async () => {
    ...
    const res = await getReceipt(
      new Request(`http://localhost/api/protocol/x402/receipts/${receiptId}`),
      {
