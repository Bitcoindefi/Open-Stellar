...
function register(
  agentId: string,
  district: "data-center" | "comm-hub" = "data-center",
) {
  registerAgent({
    agentId,
...
describe("GET /api/leaderboard", () => {
  ...
  it("filters district view and sorts that district by task completions", async () => {
    ...
    const res = await GET(
      new Request(
        "http://localhost/api/leaderboard?view=district&district=data-center",
      ),
    );
    const body = await res.json();
    ...
    expect(
      body.agents.every(
        (agent: { district: string }) => agent.district === "data-center",
      ),
    ).toBe(true);
  });
