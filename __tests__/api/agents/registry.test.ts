...
describe("agent registry API", () => {
  it("registers, lists, filters, and retrieves agents", async () => {
    ...
    const list = await GET(
      new Request(
        "http://localhost/api/agents?district=data-center&status=active&skill=log-analysis",
      ),
    );
    ...
    const single = await getAgent(
      new Request("http://localhost/api/agents/nexus-7"),
  ...
  it("updates capabilities and deregisters agents", async () => {
    ...
    const update = await putCapabilities(
      new Request("http://localhost/api/agents/nexus-7/capabilities", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
    ...
    const deleted = await DELETE(
      new Request("http://localhost/api/agents/nexus-7", { method: "DELETE" }),
      agentContext("nexus-7"),
    );
    ...
    await POST(
      new Request("http://localhost/api/agents", {
        ...
        body: JSON.stringify({
          ...manifest,
          agentId: "nexus-8",
          capabilities: ["payment", "translation"],
        }),
      }),
    );

    const res = await GET(
      new Request("http://localhost/api/agents?capability=DATA-INDEXING"),
    );
    const body = await res.json();
    expect(body.agents).toHaveLength(1);
    expect(body.agents[0].agentId).toBe("nexus-7");

    const noneRes = await GET(
      new Request("http://localhost/api/agents?capability=unknown"),
    );
