...
describe("GET /api/agents/leaderboard", () => {
  ...
  it("returns 400 Bad Request if limit > 100", async () => {
    const req = new Request(
      "http://localhost/api/agents/leaderboard?limit=101",
    );
  ...
  it("sorts by reputation", async () => {
    ...
    const req = new Request(
      `http://localhost/api/agents/leaderboard?sort=reputation&limit=3`,
    );
    const res = await GET(req);
    const data = await res.json();
  ...
  it("sorts by tasks and handles offset correctly", async () => {
    ...
    const req = new Request(
      `http://localhost/api/agents/leaderboard?sort=tasks&offset=2&limit=2`,
    );
    const res = await GET(req);
    const data = await res.json();
