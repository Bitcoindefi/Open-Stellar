...
describe("agent dependency graph API", () => {
  ...
  it("returns a single direct dependency", async () => {
    ...
    expect(await response.json()).toEqual({
      agentId: "agent-a",
      dependencies: [{ agentId: "agent-b", depth: 1, dependencies: [] }],
      totalCount: 1,
      maxDepth: 1,
    });
  });
      ...
      dependencies: [
        {
          agentId: "agent-b",
          depth: 1,
          dependencies: [{ agentId: "agent-c", depth: 2, dependencies: [] }],
        },
      ],
      totalCount: 2,
      maxDepth: 2,
      ...
      dependencies: [
        {
          agentId: "agent-b",
          depth: 1,
          dependencies: [{ agentId: "agent-d", depth: 2, dependencies: [] }],
        },
        {
          agentId: "agent-c",
          depth: 1,
      ...
      dependents: [
        {
          agentId: "agent-b",
          depth: 1,
          dependents: [{ agentId: "agent-a", depth: 2, dependents: [] }],
        },
        {
          agentId: "agent-c",
          depth: 1,
  ...
  it("limits traversal to maxDepth", async () => {
    ...
    const response = await getDependencies(
      new Request(
        "http://localhost/api/agents/agent-a/dependencies?maxDepth=1",
      ),
      context("agent-a"),
    );
  ...
  it("returns an empty traversal when maxDepth is zero", async () => {
    ...
    const response = await getDependencies(
      new Request(
        "http://localhost/api/agents/agent-a/dependencies?maxDepth=0",
      ),
      context("agent-a"),
    );
  ...
  it.each(["invalid", "-1"])(
    ...
    async (maxDepth) => {
      ...
      const response = await getDependencies(
        new Request(
          `http://localhost/api/agents/agent-a/dependencies?flat=true&maxDepth=${maxDepth}`,
        ),
        context("agent-a"),
      );
  ...
  it("rejects maxDepth values above ten", async () => {
    ...
    const response = await getDependencies(
      new Request(
        "http://localhost/api/agents/agent-a/dependencies?maxDepth=11",
      ),
      context("agent-a"),
    );
...
describe("agent dependency registration", () => {
  ...
  it("rejects an unknown dependency", () => {
    expect(() => registerAgent(manifest("agent-a", ["missing"]))).toThrow(
      "dependency agent not found: missing",
    );
  });

  it("rejects a self-dependency", () => {
    expect(() => registerAgent(manifest("agent-a", ["agent-a"]))).toThrow(
  ...
  it("rejects a cycle when an existing agent is re-registered", () => {
    ...
    expect(() => registerAgent(manifest("agent-a", ["agent-b"]))).toThrow(
      "agent dependencies cannot contain a cycle",
    );
  });
});
