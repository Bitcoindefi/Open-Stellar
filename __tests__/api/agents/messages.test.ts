...
import {
  GET as getMessages,
  POST as postMessage,
} from "@/app/api/agents/[id]/messages/route";
import {
  GET as getDistrictMessages,
  POST as postDistrictMessage,
} from "@/app/api/districts/[districtId]/broadcast/route";
import {
  listAgentMessages,
...
describe("agent messaging protocol", () => {
  ...
  it("notifies subscribed agents over the message bus", () => {
    ...
    const unsubscribe = subscribeToAgentMessages("bot-2", (message) =>
  ...
  it("serves direct message POST and list APIs", async () => {
    const post = await postMessage(
      new Request("http://localhost/api/agents/bot-3/messages", {
        method: "POST",
  ...
  it("broadcasts district messages", async () => {
    const post = await postDistrictMessage(
      new Request("http://localhost/api/districts/data-center/broadcast", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          fromAgentId: "bot-0",
    ...
    const list = await getDistrictMessages(
      new Request("http://localhost/api/districts/data-center/broadcast"),
      districtContext("data-center"),
    );
    const listBody = await list.json();
  ...
  it("streams incoming messages as SSE frames", async () => {
    const res = await getMessages(
      new Request("http://localhost/api/agents/bot-4/messages?stream=1"),
      agentContext("bot-4"),
    );
