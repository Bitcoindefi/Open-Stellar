...
describe("GET /api/agents/:id/capabilities/compat", () => {
  ...
  it("returns 404 for unknown agent", async () => {
    const req = new Request(
      "http://localhost/api/agents/unknown/capabilities/compat?callerVersion=1.0.0",
    );
  ...
  it("returns 400 when callerVersion is missing", async () => {
    ...
    const req = new Request(
      "http://localhost/api/agents/bot-compat/capabilities/compat",
    );
  ...
  it("returns compatible=true when all skills match caller version", async () => {
    ...
    const req = new Request(
      "http://localhost/api/agents/bot-compat/capabilities/compat?callerVersion=2.5.0",
    );
    const res = await GET(req, agentContext("bot-compat"));
  ...
  it("returns compatible=false for skills with incompatible minCallerVersion", async () => {
    ...
    const req = new Request(
      "http://localhost/api/agents/bot-incompat/capabilities/compat?callerVersion=1.0.0",
    );
  ...
  it("returns compatible=true for skills without minCallerVersion regardless of callerVersion", async () => {
    registerAgent({
      ...
      district: "data-center",
      capabilities: ["translate"],
      skillVersions: [{ id: "translate", version: "2.1.0" }],
      status: "active",
      endpoint: "http://test",
      x402: { accepts: true },
    });
    ...
    const req = new Request(
      "http://localhost/api/agents/bot-no-min/capabilities/compat?callerVersion=0.1.0",
    );
  ...
  it("defaults missing version to 1.0.0", async () => {
    registerAgent({
      ...
      district: "data-center",
      capabilities: ["translate"],
      skillVersions: [{ id: "translate", minCallerVersion: ">=1.0.0" }],
      status: "active",
      endpoint: "http://test",
      x402: { accepts: true },
    });
    ...
    const req = new Request(
      "http://localhost/api/agents/bot-default/capabilities/compat?callerVersion=1.0.0",
    );
  ...
  it("handles semver range operators correctly", async () => {
    ...
    const req = new Request(
      "http://localhost/api/agents/bot-semver/capabilities/compat?callerVersion=1.2.3",
    );
