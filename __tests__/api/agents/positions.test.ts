...
beforeEach(() => {
  ...
  resetAgentPositionStoreForTests();
  setAgentPositionForTests("bot-1", {
    pixelX: 10,
    pixelY: 20,
    targetX: 10,
    targetY: 20,
  ...
  setAgentPositionForTests("bot-2", {
    pixelX: 100,
    pixelY: 200,
    targetX: 100,
    targetY: 200,
...
describe("agent position store", () => {
  it("moves only the requested agent and publishes a single-agent delta", () => {
    ...
    expect(getAgentPosition("bot-2")).toMatchObject({
      pixelX: 100,
      pixelY: 200,
    });
  ...
  it("keeps history bounded to the latest 1000 entries", { timeout: 40000 }, () => {
  ...
  it("accepts a 500-char agentId but stores it with a 200-char key", () => {
    ...
    setAgentPositionForTests(hugeId, {
      pixelX: 0,
      pixelY: 0,
      targetX: 0,
      targetY: 0,
...
describe("POST /api/agents/[id]/move", () => {
  it("requires MOLTBOT_GATEWAY_TOKEN bearer auth", async () => {
    ...
    const res = await moveAgent(
      new Request("http://localhost/api/agents/bot-1/move", {
        method: "POST",
