...
describe("district map route", () => {
  it("returns all configured districts with 1 unlocked and 2 locked with correct status fields", async () => {
    ...
    const dataCenter = data.districts.find(
      (d: { id: string }) => d.id === "data-center",
    );
    ...
    const commHub = data.districts.find(
      (d: { id: string }) => d.id === "comm-hub",
    );
    ...
    const processing = data.districts.find(
      (d: { id: string }) => d.id === "processing",
    );
  ...
  it("unlocks multiple districts when XP surpasses several thresholds at once", () => {
    ...
    expect(unlocked.map((d) => d.id)).toEqual([
      "data-center",
      "comm-hub",
      "processing",
    ]);
  ...
  it("xpRequired on each district matches the registry threshold", async () => {
    ...
    for (let i = 0; i < DISTRICT_REGISTRY.length; i++) {
      expect(result.districts[i].xpRequired).toBe(
        DISTRICT_REGISTRY[i].xpRequired,
      );
    }
  });
});
