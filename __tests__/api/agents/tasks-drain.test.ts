...
import {
  POST as createPost,
  DELETE as purgeDel,
} from "@/app/api/agents/[id]/tasks/route";
import {
  registerAgent,
...
describe("POST /api/agents/:id/tasks/drain", () => {
  ...
  it("returns 409 on concurrent drain attempts", async () => {
    ...
    const req1 = new Request(
      "http://localhost/api/agents/agent-1/tasks/drain",
      {
        method: "POST",
      },
    );
  ...
  it("isolates drain between agents", async () => {
    ...
    const req2 = new Request(
      "http://localhost/api/agents/agent-2/tasks/drain",
      {
        method: "POST",
      },
    );
