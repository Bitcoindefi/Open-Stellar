...
describe("GET /api/agents/:id/capabilities", () => {
  beforeEach(() => {
    resetAgentRegistryForTests();
    vi.clearAllMocks();
    vi.spyOn(questLeaderboard, "getAgentQuestStats").mockReturnValue({
      questsCompleted: 0,
      xpFromQuests: 0,
      rank: null,
    });
  ...
  it("returns full agent capabilities with actual registered versions", async () => {
      ...
      badges: [
        {
          id: "badge-speed",
          rarity: "rare",
          awardedAt: "2026-06-25T08:00:00Z",
        },
      ],
    });
    ...
    const req = new Request(
      "http://localhost/api/agents/bot-full/capabilities",
    );
    ...
    expect(json).toEqual({
      ...
      districts: [
        { districtId: "data-center", unlockedAt: agent.registeredAt },
      ],
  ...
  it("returns default version 1.0.0 for skills without explicit version", async () => {
    ...
    const req = new Request(
      "http://localhost/api/agents/bot-default/capabilities",
    );
    const res = await GET(req, {
      params: Promise.resolve({ id: "bot-default" }),
    });
    expect(res.status).toBe(200);
  ...
  it("returns empty arrays for agent with no skills/badges", async () => {
    ...
    const req = new Request(
      "http://localhost/api/agents/bot-empty/capabilities",
    );
    const res = await GET(req, {
