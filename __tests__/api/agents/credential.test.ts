...
describe("agent reputation credentials", () => {
  it("issues a credential with export and share metadata", async () => {
    ...
    const req = new Request(
      `http://localhost/api/agents/${agentId}/credential`,
      {
        method: "POST",
    ...
    expect(data.credential.tasksCompleted).toBe(
      data.credential.reputation.metrics.tasksCompleted,
    );
    expect(data.credential.successRate).toBe(data.credential.agent.successRate);
    expect(data.credential.signature).toBe(
      data.credential.proof.attestationHash,
    );
  ...
  it("returns the latest issued credential as JSON", async () => {
    ...
    await issueCredential(
      new Request(`http://localhost/api/agents/${agentId}/credential`, {
        method: "POST",
        body: JSON.stringify({}),
        headers: { "Content-Type": "application/json" },
      }),
    ...
    const res = await getLatestCredential(
      new Request(`http://localhost/api/agents/${agentId}/credential/latest`),
      {
        params: Promise.resolve({ id: agentId }),
      },
    );
    ...
    expect(data.ok).toBe(true);
    expect(data.credential.agent.id).toBe(agentId);
    expect(data.credential.links.shareUrl).toBe(
      `http://localhost/credential/${agentId}`,
    );
    expect(data.credential.links.explorerUrl).toBeNull();
  ...
  it("returns 404 when no credential has been issued", async () => {
    const res = await getLatestCredential(
      new Request("http://localhost/api/agents/not-issued/credential/latest"),
      {
