...
import {
  POST as registerSkillPost,
  GET as listSkillsGet,
...
describe("Skills API", () => {
  ...
  describe("POST /api/agents/[id]/skills", () => {
    it("registers a new skill", async () => {
      const req = await mockRequest(
        "http://localhost:3000/api/agents/agent-001/skills",
        {
          method: "POST",
    ...
    it("returns 400 for invalid input", async () => {
      const req = await mockRequest(
        "http://localhost:3000/api/agents/agent-002/skills",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  describe("GET /api/agents/[id]/skills", () => {
    it("lists all skills for an agent", async () => {
      ...
      const req1 = await mockRequest(
        "http://localhost:3000/api/agents/agent-003/skills",
        {
          method: "POST",
          body: JSON.stringify({
            skillId: "skill-1",
      ...
      // List skills
      const listReq = await mockRequest(
        "http://localhost:3000/api/agents/agent-003/skills",
      );
    ...
    it("returns empty array for agent with no skills", async () => {
      const req = await mockRequest(
        "http://localhost:3000/api/agents/agent-999/skills",
      );
  ...
  describe("GET /api/skills", () => {
    beforeEach(async () => {
      ...
      for (const skill of skills) {
        const req = await mockRequest(
          `http://localhost:3000/api/agents/${skill.agentId}/skills`,
          {
            method: "POST",
    ...
    it("filters by search query", async () => {
      const req = await mockRequest(
        "http://localhost:3000/api/skills?q=payment",
      );
      const response = await searchSkillsGet(req);
      const data = await response.json();
    ...
    it("filters by maxPrice", async () => {
      const req = await mockRequest(
        "http://localhost:3000/api/skills?maxPrice=5",
      );
      const response = await searchSkillsGet(req);
      const data = await response.json();
      ...
      expect(data.skills).toHaveLength(2);
      expect(
        data.skills.every((s: { priceXlm: number }) => s.priceXlm <= 5),
      ).toBe(true);
    });
  });
  ...
  describe("GET /api/agents/[id]/skills/[skillId]/invoke", () => {
    beforeEach(async () => {
      // Register a skill
      const req = await mockRequest(
        "http://localhost:3000/api/agents/agent-pay-1/skills",
        {
          method: "POST",
          body: JSON.stringify({
  ...
  describe("POST /api/agents/[id]/skills/[skillId]/invoke", () => {
    beforeEach(async () => {
      // Register a skill
      const req = await mockRequest(
        "http://localhost:3000/api/agents/agent-pay-2/skills",
        {
          method: "POST",
          body: JSON.stringify({
