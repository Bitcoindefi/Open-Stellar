...
describe("/api/tasks", () => {
  ...
  it("enqueues and retrieves a prioritized task", async () => {
    const createRes = await enqueueTask(
  ...
  it("lists tasks for an agent in priority order", async () => {
    for (const task of [
      { type: "low-work", priority: "low" },
      { type: "critical-work", priority: "critical" },
    ]) {
      await enqueueTask(
        new Request("http://localhost/api/tasks", {
          method: "POST",
    ...
    const res = await listTasks(
      new Request("http://localhost/api/tasks?agent=bot-1"),
    );
    const data = await res.json();
    expect(data.tasks.map((task: { type: string }) => task.type)).toEqual([
      "critical-work",
      "low-work",
    ]);
  });
  ...
  it("cancels pending tasks", async () => {
    ...
    const res = await cancelTask(
      new Request(`http://localhost/api/tasks/${created.task.id}/cancel`, {
        method: "POST",
      }),
      context(created.task.id),
    );
  ...
  it("backs off retries then moves exhausted tasks to dead-letter", async () => {
    ...
    const firstFail = await failTask(
      new Request(`http://localhost/api/tasks/${created.task.id}/fail`, {
        method: "POST",
    ...
    const secondFail = await failTask(
      new Request(`http://localhost/api/tasks/${created.task.id}/fail`, {
        method: "POST",
  ...
  it("persists dead-letter entries and reloads them from file", async () => {
    ...
    const failRes = await failTask(
      new Request("http://localhost/api/tasks/persisted-task/fail", {
        method: "POST",
        body: JSON.stringify({ error: "permanent" }),
      }),
    ...
    const persisted = JSON.parse(
      readFileSync(process.env.TASK_DLQ_FILE!, "utf8"),
    );
    ...
    resetTaskQueueForTests();
    loadDeadLetterQueueForTests();
    const listRes = await listDeadLetterTasks(
      new Request("http://localhost/api/tasks/dead-letter"),
    );
  ...
  it("lists, retries, and discards dead-letter tasks via dedicated endpoints", async () => {
    for (const id of ["dlq-1", "dlq-2"]) {
      ...
      await failTask(
        new Request(`http://localhost/api/tasks/${id}/fail`, {
          method: "POST",
    ...
    const listRes = await listDeadLetterTasks(
      new Request("http://localhost/api/tasks/dead-letter?limit=1&offset=1"),
    );
    const list = await listRes.json();
    ...
    expect(list.tasks[0]).toMatchObject({
      errorMessage: "dlq-2 failed",
      retryCount: 0,
    });

    const retryRes = await retryDeadLetterTask(
      new Request("http://localhost/api/tasks/dead-letter/dlq-1/retry", {
        method: "POST",
      }),
      context("dlq-1"),
    );
    ...
    const discardRes = await discardDeadLetterTask(
      new Request("http://localhost/api/tasks/dead-letter/dlq-2", {
        method: "DELETE",
      }),
      context("dlq-2"),
    );
    ...
    const notFoundRes = await discardDeadLetterTask(
      new Request("http://localhost/api/tasks/dead-letter/missing", {
        method: "DELETE",
      }),
      context("missing"),
    );
