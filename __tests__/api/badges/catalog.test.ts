...
describe("GET /api/badges", () => {
  ...
  it("earnedByCount counts distinct agents with that badge", async () => {
      ...
      badges: [
        {
          id: "first-quest",
          rarity: "common",
          awardedAt: "2026-05-02T00:00:00.000Z",
        },
      ],
    });
    ...
    const json = await res.json();
    const fq = json.find(
      (e: { badgeId: string }) => e.badgeId === "first-quest",
    );
    ...
    upsertReputationMetrics("agent-dup", {
      badges: [
        ...
        },
        {
          id: "first-quest",
          rarity: "common",
          awardedAt: "2026-05-03T00:00:00.000Z",
        },
      ],
    });
  ...
  it("filters catalog by rarity", async () => {
    const res = await GET(
      new Request("http://localhost/api/badges?rarity=common"),
    );
    expect(res.status).toBe(200);
    const json = await res.json();
    expect(json.length).toBeGreaterThan(0);
    expect(json.every((e: { rarity: string }) => e.rarity === "common")).toBe(
      true,
    );
  });
  ...
  it("returns 400 for invalid rarity", async () => {
    const res = await GET(
      new Request("http://localhost/api/badges?rarity=mythic"),
    );
