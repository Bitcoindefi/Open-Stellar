...
function makeCreateRequest(
  body: object,
  questId = "weekly-onboard-marketplace-service",
): Request {
...
function makeUpdateRequest(
  ...
  return new Request(
    `http://localhost/api/quests/${questId}/subtasks/${subtaskId}`,
    {
      method: "PATCH",
      body: JSON.stringify(body),
      headers: { "Content-Type": "application/json" },
...
function makeApplyRequest(actorId: string): Request {
  return new Request(
    "http://localhost/api/quests/weekly-onboard-marketplace-service/apply",
    {
      method: "POST",
...
describe("Quest Subtasks API", () => {
  ...
  it("PATCH updates status and assignment; completing all sub-tasks marks parent quest complete", async () => {
    ...
    expect(applyFailData.error).toContain(
      "Cannot complete quest with pending sub-tasks",
    );
  ...
  it("PATCH completes a subtask with no dependencies", async () => {
    ...
    const resCreate = await createSubTaskRoute(
      makeCreateRequest(
        { title: "Complete independent task" },
        independentQuestId,
      ),
  ...
  it("PATCH blocks completion when one dependency is incomplete", async () => {
    ...
    const dependentRes = await createSubTaskRoute(
      makeCreateRequest(
        { title: "Complete dependent task", dependsOn: [prerequisite.id] },
        dependentQuestId,
      ),
      context(dependentQuestId),
    );
  ...
  it("PATCH blocks completion with two dependencies when one is incomplete and one is missing", async () => {
    ...
    const incompletePrerequisite = (await incompletePrerequisiteRes.json())
      .subTask;
    const dependentRes = await createSubTaskRoute(
      makeCreateRequest(
        {
