...
async function registerWebhook(
  ...
  const res = await POST(
    webhookRequest({ url, events: ["agent.status", "quest.completed"] }),
  );
  return {
    res,
...
describe("webhook API", () => {
  ...
  beforeEach(() => {
    ...
    setWebhookDeliveryLogPathForTests(
      join(testDir, "webhook-delivery-log.jsonl"),
    );
  ...
  it("rejects invalid webhook URLs", async () => {
    const res = await POST(
      webhookRequest({ url: "not a url", events: ["agent.status"] }),
    );
  ...
  it("rejects unknown event types", async () => {
    const res = await POST(
      webhookRequest({
        ...
        events: ["agent.status", "nonsense.event"],
      }),
    );
    const data = await res.json();
  ...
  it("deletes a registered webhook", async () => {
    ...
    const deleted = await DELETE(
      new Request(`http://localhost/api/webhooks/${registered.id}`, {
        method: "DELETE",
      }),
  ...
  it("rotates a webhook secret without changing public registration fields", async () => {
    ...
    const rotated = await ROTATE_POST(
      new Request(`http://localhost/api/webhooks/${registered.id}/rotate`, {
        method: "POST",
      }),
      context(registered.id),
    );
    const rotatedData = (await rotated.json()) as {
      id: string;
      secret: string;
  ...
  it("returns 404 when rotating an unknown webhook", async () => {
    const res = await ROTATE_POST(
      new Request("http://localhost/api/webhooks/wh_missing/rotate", {
        method: "POST",
      }),
      context("wh_missing"),
    );
  ...
  it("replays only events matching the webhook event registrations", async () => {
    ...
    const registeredResponse = await POST(
      webhookRequest({
        url: "https://partner.example/webhooks/open-stellar",
        events: ["agent.status"],
      }),
  ...
  it("replays all matching webhook events in the requested window", async () => {
    ...
    const data = await res.json();
    const replayedIds = fetchMock.mock.calls.map(
      (call) => JSON.parse(call[1]?.body as string).payload.id,
    );
  ...
  it("retries once when delivery receives an error response", async () => {
    ...
    expect(fetchMock.mock.calls[1][0]).toBe(
      "https://partner.example/webhooks/open-stellar",
    );
  ...
  it("lists two deliveries newest-first without exposing secrets", async () => {
    ...
    const res = await DELIVERIES_GET(
      new Request(
        `http://localhost/api/webhooks/${registered.id}/deliveries?limit=20`,
      ),
      context(registered.id),
    );
    const data = await res.json();
    ...
    expect(Date.parse(data.deliveries[0].deliveredAt)).toBeGreaterThanOrEqual(
      Date.parse(data.deliveries[1].deliveredAt),
    );
    expect(data.deliveries[0]).not.toHaveProperty("secret");

    const limited = await DELIVERIES_GET(
      new Request(
        `http://localhost/api/webhooks/${registered.id}/deliveries?limit=1`,
      ),
      context(registered.id),
    );
