...
describe("GET /api/cron/passport-expiry", () => {
  ...
  it("returns 200 with correct Bearer token", async () => {
    ...
    seedPassportExpiryRecord(
      makePassport({ expiresAt: new Date(past).toISOString() }),
    );
  ...
  it("returns correct expired count for multiple passports", async () => {
    ...
    seedPassportExpiryRecord(
      makePassport({ agentId: "a1", expiresAt: new Date(past).toISOString() }),
    );
    seedPassportExpiryRecord(
      makePassport({ agentId: "a2", expiresAt: new Date(past).toISOString() }),
    );
    seedPassportExpiryRecord(
      makePassport({
        agentId: "a3",
