...
const EXPECTED_SHA256 =
  "33023ba554ba20b1920043a14698d4ad182eb86611a7bdd9965767975b69a8cc";

function fail(msg) {
  console.error(`\n❌ passport-spec-guard: ${msg}\n`);
  console.error(
    "If this change is an INTENTIONAL contract-spec upgrade, update",
  );
  console.error(
    "EXPECTED_SHA256 in scripts/check-passport-spec.mjs in the same PR.\n",
  );
  process.exit(1);
}
...
if (specs.length === 0)
  fail("no base64 spec entries found inside ContractSpec([...])");
...
for (const s of specs) {
  ...
  if (roundtrip !== s) {
    fail(
      `invalid base64 in a ContractSpec entry (starts with "${s.slice(0, 32)}...")`,
    );
  }
}
...
console.log(
  `✅ passport-spec-guard: ${specs.length} ContractSpec entries valid and unchanged.`,
);
