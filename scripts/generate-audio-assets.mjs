...
function addSine(buf, freq, ampFn, opts = {}) {
  ...
  const lenSamples = Math.round(
    (opts.durSec ?? buf.length / SAMPLE_RATE) * SAMPLE_RATE,
  );
...
function fadeInOut(buf, fadeSec) {
  const fadeSamples = Math.min(
    Math.round(fadeSec * SAMPLE_RATE),
...
function writeTrack(relativePath, buf) {
  ...
  console.log(
    `wrote ${relativePath} (${(buf.length / SAMPLE_RATE).toFixed(2)}s)`,
  );
}
  ...
  return buildLoopable(LOOP_DURATION, CROSSFADE, (buf, total) => {
    ...
    for (let cycle = 0; cycle < 2; cycle++) {
      for (const freq of notes) {
        addSine(buf, freq, (lt, ld) => 0.18 * Math.exp(-lt / (ld * 0.6)), {
          startSec: t,
          durSec: noteDur,
        });
        t += noteDur;
...
function processingDay(rng) {
  return buildLoopable(LOOP_DURATION, CROSSFADE, (buf, total) => {
    ...
    addPulseTrain(buf, 0.25, total, 0.07, {
      durSec: 0.02,
      noiseAmt: 0.3,
      freq: 600,
      rng,
...
function processingNight(rng) {
  return buildLoopable(LOOP_DURATION, CROSSFADE, (buf, total) => {
    ...
    addPulseTrain(buf, 0.5, total, 0.32, {
      durSec: 0.15,
      freq: 110,
      freqEnd: 45,
      decayFactor: 0.18,
    });
    addPulseTrain(buf, 1.0, total, 0.18, {
      durSec: 0.3,
      freq: 55,
      decayFactor: 0.4,
    });
    addPulseTrain(buf, 0.25, total, 0.06, {
      durSec: 0.03,
      noiseAmt: 1,
      rng,
      highpassHz: 5000,
...
function defenseDay(rng) {
  return buildLoopable(LOOP_DURATION, CROSSFADE, (buf, total) => {
    addPulseTrain(buf, 1.0, total, 0.26, {
      durSec: 0.1,
      freq: 95,
      decayFactor: 0.25,
    }); // marching kick
    addPulseTrain(buf, 1.0, total, 0.28, {
      durSec: 0.12,
      noiseAmt: 1,
      rng,
      lowpassHz: 4000,
...
function defenseNight(_rng) {
  return buildLoopable(LOOP_DURATION, CROSSFADE, (buf, _total) => {
    ...
    for (const at of [0.6, 2.6]) {
      ...
      addTransient(buf, at + 0.12, 0.05, 0.11, {
        freq: 1800,
        decayFactor: 0.2,
      });
      addTransient(buf, at + 0.24, 0.05, 0.05, {
        freq: 1800,
        decayFactor: 0.2,
      });
    }
  ...
  return buildLoopable(LOOP_DURATION, CROSSFADE, (buf, total) => {
    for (const freq of [440, 554.37, 659.25]) {
      addSine(
        buf,
        freq,
        (t) => 0.045 + 0.015 * Math.sin(2 * Math.PI * 0.5 * t + freq),
        { freqEnd: freq * 1.004 },
      );
    }
    addNoiseBand(
      buf,
      rng,
      (t) => 0.03 * (0.5 + 0.5 * Math.sin((2 * Math.PI * t) / total)),
      {
        lowpassHz: 3500,
        highpassHz: 1200,
      },
...
function taskComplete() {
  return buildOneShot(0.05, (buf) => {
    addSine(buf, 880, (t, d) => 0.55 * Math.exp(-t / (d * 0.35)), {
      freqEnd: 1320,
    });
  });
}
...
function paymentReceived() {
  return buildOneShot(0.3, (buf) => {
    ...
    let t = 0;
    for (const freq of notes) {
      addSine(buf, freq, (lt, ld) => 0.32 * Math.exp(-lt / (ld * 0.5)), {
        startSec: t,
        durSec: 0.13,
      });
      t += 0.05;
    }
    addNoiseBand(buf, makeRng(7), (t) => 0.05 * Math.exp(-t / 0.1), {
      highpassHz: 7000,
      durSec: 0.3,
    });
  });
...
function levelUp() {
  return buildOneShot(0.8, (buf) => {
    ...
    let t = 0;
    for (const freq of notes) {
      addSine(buf, freq, (lt, ld) => 0.45 * Math.exp(-lt / (ld * 0.7)), {
        startSec: t,
        durSec: 0.22,
      });
      addSine(buf, freq * 2, (lt, ld) => 0.08 * Math.exp(-lt / (ld * 0.5)), {
        startSec: t,
        durSec: 0.22,
      });
      t += 0.19;
...
function badgeUnlock() {
  return buildOneShot(1.2, (buf) => {
    ...
    for (const freq of chord) {
      addSine(buf, freq, (t, d) => 0.22 * Math.exp(-t / (d * 0.9)), {
        durSec: 0.9,
      });
      addSine(buf, freq * 1.003, (t, d) => 0.14 * Math.exp(-t / (d * 0.9)), {
        durSec: 0.9,
      }); // light chorus detune
    }
    ...
    for (const freq of flourish) {
      addSine(buf, freq, (lt, ld) => 0.3 * Math.exp(-lt / (ld * 0.6)), {
        startSec: t,
        durSec: 0.16,
      });
      t += 0.1;
...
function districtWin() {
  return buildOneShot(2.0, (buf) => {
    ...
    for (const freq of pad)
      addSine(buf, freq, (t, d) => 0.08 * Math.exp(-t / (d * 0.8)), {
        durSec: 2.0,
      });
    ...
    for (const note of melody) {
      addSine(buf, note.freq, (lt, ld) => 0.38 * Math.exp(-lt / (ld * 0.6)), {
        startSec: note.at,
        durSec: note.dur,
      });
      addSine(
        buf,
        note.freq * 2,
        (lt, ld) => 0.07 * Math.exp(-lt / (ld * 0.5)),
        { startSec: note.at, durSec: note.dur },
      );
    }
  });
}
...
function agentError() {
  return buildOneShot(0.2, (buf) => {
    addSquare(buf, 320, (t, d) => 0.32 * Math.exp(-t / (d * 0.5)), {
      durSec: 0.09,
      freqEnd: 320,
    });
    addSquare(buf, 220, (t, d) => 0.32 * Math.exp(-t / (d * 0.5)), {
      startSec: 0.1,
      durSec: 0.09,
    });
  });
