...
async function askNetwork(rl) {
  while (true) {
    const raw = await rl.question(
      "Stellar network — [1] testnet  [2] mainnet  (default: 1): ",
    );
...
export async function runWizard({ yes = false, cwd = process.cwd() } = {}) {
  ...
  if (yes) {
    ...
    console.log(
      `  Admin wallet : (set ADMIN_WALLET_ADDRESS in .env.local before running)`,
    );
  ...
  try {
    ...
    while (true) {
      ...
      if (!validateDbUrl(url)) {
        console.log(
          "   Warning: URL does not look like a postgres:// connection string.",
        );
      }
    ...
    while (true) {
      const wallet = await ask(
        rl,
        "4. Admin wallet address (Stellar G... public key)",
      );
      ...
      console.log(
        "   Invalid Stellar address: must start with 'G' and be exactly 56 characters.",
      );
    }
