...
import {
  getCloudAgentConfig,
  provisionCloudAgent,
  updateCloudAgentResult,
} from "@/lib/agent-runtime/cloud-agents";
import {
...
function getConfig(id: string, req: Request) {
  return (
    getCloudAgentConfig(id) ??
    provisionCloudAgent({ name: id, queueMode: "post" }, req)
  );
}
...
async function reasonAboutTask(task: string, model: string): Promise<string> {
  ...
  if (!res.ok)
    return `Claude API unavailable (${res.status}); queued task for retry: ${task.slice(0, 120)}`;
...
export async function POST(req: Request, context: RouteContext) {
  ...
  const rawTask = String(
    body.task || body.title || body.prompt || "Process orchestrator task",
  );
  ...
  if (health.degraded)
    console.warn(`[agent-health] Invoking degraded cloud agent ${config.id}`);
  recordAgentInvocation(config.id);
  recordAgentHeartbeat(config.id, {
    status: health.degraded ? "degraded" : "working",
    cpu: 32,
    memory: 42,
    currentTask: task,
    autoRestart: true,
  ...
  publishSystemEvent({
    type: "task.started",
    agentId: config.id,
    task: { id: taskId, title: task, district: config.district },
  });
  ...
  try {
    ...
    recordAgentHeartbeat(config.id, {
      status: getAgentHealthSummary(config.id).degraded ? "degraded" : "active",
      cpu: 8,
      memory: 24,
      currentTask: summary,
      autoRestart: true,
    ...
    publishSystemEvent({
      ...
      agentId: config.id,
      taskId,
      result: { summary, durationMs: Date.now() - started },
    });

    return NextResponse.json(
      { ok: true, agentId: config.id, taskId, result: { summary } },
      { headers: { "Cache-Control": "no-store" } },
    );
  } catch (error) {
    const failure = recordAgentExecutionError({
      agentId: config.id,
      error,
      taskExcerpt: task,
    });
    recordAgentHeartbeat(config.id, {
      status: failure.degraded ? "degraded" : "error",
      cpu: 8,
      memory: 24,
      currentTask: task,
      autoRestart: true,
...
export async function GET(req: Request, context: RouteContext) {
  ...
  const stream = new ReadableStream<Uint8Array>({
    start(controller) {
      const send = () => {
        const health = recordAgentHeartbeat(config.id, {
          status: "active",
          cpu: 6,
          memory: 20,
          currentTask: "SSE heartbeat",
          autoRestart: true,
        });
        controller.enqueue(
          encoder.encode(
