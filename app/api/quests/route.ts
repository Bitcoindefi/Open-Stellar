...
export async function GET(req: Request) {
  ...
  const generated = isMockMode()
    ? getQuests()
    : buildQuests(getMockQuestStats());
  ...
  const quests = includeExpired
    ? merged
    : merged.filter((q) => q.status !== "expired");
...
export async function POST(req: Request) {
  try {
    ...
    const unlocksQuestId =
      typeof body.unlocksQuestId === "string" && body.unlocksQuestId.trim()
        ? body.unlocksQuestId.trim()
        : undefined;
    const minReputation =
      body.minReputation !== undefined ? Number(body.minReputation) : undefined;
    ...
    return NextResponse.json(
      {
        ok: false,
        error: error instanceof Error ? error.message : "Failed creating quest",
      },
      { status: 500 },
    );
  }
