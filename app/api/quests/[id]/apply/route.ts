...
import {
  hasClaimedQuest,
  markQuestClaimed,
} from "@/lib/gamification/quest-completions";
...
export async function POST(req: Request, context: QuestApplyContext) {
  ...
  if (!quest) {
    return NextResponse.json(
      { ok: false, error: "Quest not found" },
      { status: 404 },
    );
  }
  ...
  if (
    ...
    quest.subTasks.some((st) => st.status !== "done")
  ) {
    return NextResponse.json(
  ...
  if (
    quest.minReputation !== undefined &&
    reputation.score < quest.minReputation
  ) {
    return NextResponse.json(
      {
        ok: false,
