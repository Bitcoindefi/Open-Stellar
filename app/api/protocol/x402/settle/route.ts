...
} from "@/lib/protocols/subscription-anchor";
...
function subscriptionMatchesQuote(
  ...
  return (
    subscription.agentId === quote.payer &&
    subscription.serviceId === quote.serviceId
  );
}
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const chain =
      body.chain === "bnb" || body.chain === "base" || body.chain === "stellar"
        ? body.chain
        : "stellar";
    ...
    const subscription = subscriptionId
      ? getX402SubscriptionById(subscriptionId)
      : undefined;
    ...
    if (subscriptionId && !subscriptionMatchesQuote(subscription, quote)) {
      return await api.json(
        {
          ok: false,
          error: "subscriptionId does not match quote payer/service",
        },
        { status: 400 },
        {
          event: "x402.settle.rejected",
    ...
    if (isMockMode()) {
      ...
      if (agentId || paidBy) {
        awardXP(
          agentId || paidBy,
          XP_AWARDS.X402_PAYMENT_RECEIVED,
          "payment.received",
        );
        publishSystemEvent({
      ...
      return await api.json(
        { ok: true, receipt, subscriptionProof },
        undefined,
        { event: "x402.settle.mock", paymentRef, subscriptionId },
      );
    }
    ...
    if (agentId) {
      ...
      if (!gate.authorized) {
        return await api.report(
          ...
          { status: 402 },
          {
            event: "x402.settle.passport_denied",
            reason: gate.reason,
            paymentRef,
            chain,
            agentId,
    ...
    return await api.json(
      { ok: true, receipt: result.receipt, subscriptionProof },
      undefined,
      {
