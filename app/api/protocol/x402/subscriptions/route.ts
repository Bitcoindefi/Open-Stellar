...
export async function GET(req: Request) {
  ...
  return await api.json({ ok: true, ...listX402Subscriptions() }, undefined, {
    event: "x402.subscriptions.listed",
  });
}
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const subscription = createX402Subscription({
      ...
      callsPerMonth:
        body.callsPerMonth === undefined
          ? undefined
          : Number(body.callsPerMonth),
      pricePerMonth: body.pricePerMonth
        ? String(body.pricePerMonth)
        : undefined,
      walletBalanceXlm:
        body.walletBalanceXlm === undefined
          ? undefined
          : Number(body.walletBalanceXlm),
    });
      ...
      error,
      {
        ...
        error:
          error instanceof Error
            ? error.message
            : "Failed creating x402 subscription",
      },
      { status: 400 },
