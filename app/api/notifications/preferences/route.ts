...
export async function PATCH(req: Request) {
  try {
    ...
    if (!Array.isArray(body.muted)) {
      return NextResponse.json(
        { ok: false, error: "muted must be an array" },
        { status: 400 },
      );
    }

    const invalidType = body.muted.find(
      (type: unknown) => !isNotificationType(type),
    );
    if (invalidType !== undefined) {
      return NextResponse.json(
        {
          ok: false,
          error: `muted contains invalid notification type: ${String(invalidType)}`,
        },
        { status: 400 },
      );
    }
