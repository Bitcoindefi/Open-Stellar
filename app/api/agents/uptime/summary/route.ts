...
function calculateUptimePct(
  uptime: AgentUptimeSnapshot,
  checkedAtMs: number,
): number {
  ...
  const totalAgeDays = Math.max(
    1,
    Math.ceil((checkedAtMs - firstSeenMs) / DAY_MS),
  );
...
export async function GET(_req: Request) {
  ...
  const uptimesByAgent = new Map(
    listAgentUptimes(checkedAtMs).map((uptime) => [uptime.agentId, uptime]),
  );
  ...
  const totalUptimePct = uptimePercentages.reduce(
    (sum, value) => sum + value,
    0,
  );
