...
export async function GET(req: Request, context: RouteContext) {
  ...
  if (
    rarityFilter !== null &&
    !(BADGE_RARITY_VALUES as readonly string[]).includes(rarityFilter)
  ) {
    return NextResponse.json(
      {
        ok: false,
        error: `invalid rarity "${rarityFilter}"; must be one of: ${BADGE_RARITY_VALUES.join(", ")}`,
      },
      { status: 400 },
    );
  }
  ...
  let badges = (metrics.badges ?? [])
    ...
    .sort(
      (a, z) => new Date(z.earnedAt).getTime() - new Date(a.earnedAt).getTime(),
    );
