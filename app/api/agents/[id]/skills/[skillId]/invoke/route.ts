...
export async function GET(req: Request, context: RouteContext) {
  const api = createApiRouteLogger(
    req,
    "/api/agents/[id]/skills/[skillId]/invoke",
  );
  ...
  try {
    ...
    if (chain !== "stellar" && chain !== "bnb" && chain !== "base") {
      return await api.json(
        ...
        { status: 400 },
        {
          event: "skill.invoke.invalid_chain",
          agentId,
  ...
  try {
    ...
    if (!paymentRef) {
      return await api.json(
        ...
        { status: 400 },
        {
          event: "skill.invoke.missing_payment_ref",
          agentId,
          skillId: decodedSkillId,
        },
      );
    ...
    if (!txHash) {
      return await api.json(
        ...
        { status: 400 },
        {
          event: "skill.invoke.missing_tx_hash",
          agentId,
          skillId: decodedSkillId,
        },
      );
    ...
    return await api.report(
      ...
      error,
      {
        ok: false,
        error:
          error instanceof Error ? error.message : "Failed to invoke skill",
      },
      { status: 500 },
      { event: "skill.invoke.failed", agentId, skillId: decodedSkillId },
    );
