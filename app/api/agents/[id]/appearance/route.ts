...
import {
  ...
  setAgentCustomColor,
  setAgentSkin,
} from "@/lib/agents/agent-appearance-store";
import {
  ACCESSORIES,
  COLOR_CHANGE_COST_XLM,
  SKINS,
  isAccessoryUnlockedForBadges,
  isSkinUnlockedForLevel,
} from "@/lib/cosmetics";
import {
...
async function verifyTreasuryPayment(
  ...
  try {
    ...
    if (!paid)
      return {
        ok: false,
        reason: "Transaction did not pay the treasury the required amount",
      };
    return { ok: true };
  } catch (error) {
    return {
      ...
      reason:
        error instanceof Error
          ? error.message
          : "Could not verify transaction on Horizon",
    };
  }
}
  ...
  return await api.json(
    {
      ...
      agentId,
      appearance,
      treasuryAddress: PROTOCOL_TREASURY_ADDRESS || null,
      costXlm: COLOR_CHANGE_COST_XLM,
    },
  ...
  try {
    ...
    if (action === "equip-skin") {
      ...
      if (!Number.isFinite(level) || !isSkinUnlockedForLevel(skinId, level)) {
        return await api.json(
          { ok: false, error: "Skin not unlocked at reported level" },
          { status: 403 },
          { reason: "skin_locked" },
        );
      }
      const appearance = setAgentSkin(agentId, skinId);
      return await api.json({ ok: true, agentId, appearance }, undefined, {
        event: "appearance.skin_equipped",
        agentId,
        skinId,
      });
    }
    ...
    if (action === "equip-accessories") {
      const accessoryIds = Array.isArray(body.accessoryIds)
        ? (body.accessoryIds as AccessoryId[])
        : [];
      const badgeIds = Array.isArray(body.badgeIds)
        ? (body.badgeIds as string[])
        : [];

      if (!accessoryIds.every((id) => ACCESSORIES.some((a) => a.id === id))) {
        return await api.json(
          { ok: false, error: "Unknown accessory" },
          { status: 400 },
          { reason: "unknown_accessory" },
        );
      }
      const locked = accessoryIds.filter(
        (id) => !isAccessoryUnlockedForBadges(id, badgeIds),
      );
      if (locked.length > 0) {
        return await api.json(
      ...
      return await api.json({ ok: true, agentId, appearance }, undefined, {
        event: "appearance.accessories_equipped",
        agentId,
        accessoryIds,
      });
    }
    ...
    if (action === "set-color") {
      ...
      if (!HEX_COLOR.test(color)) {
        return await api.json(
          { ok: false, error: "Color must be a 6-digit hex value" },
          { status: 400 },
        ...
        return await api.json(
          {
            ok: false,
            error:
              "Treasury address not configured -- set STELLAR_TREASURY_ADDRESS",
          },
          { status: 503 },
      ...
      return await api.json({ ok: true, agentId, appearance }, undefined, {
        event: "appearance.color_purchased",
        agentId,
        color,
        txHash,
      });
    ...
    return await api.report(
      ...
      error,
      {
        ...
        error:
          error instanceof Error
            ? error.message
            : "Failed updating agent appearance",
      },
      { status: 400 },
