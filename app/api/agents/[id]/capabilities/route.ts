...
export async function GET(req: Request, context: RouteContext) {
  ...
  return NextResponse.json({
    ...
    badges: (reputation.metrics?.badges ?? []).map(
      (b: { id: string; awardedAt: string }) => ({
        id: b.id,
...
export async function PUT(req: Request, context: RouteContext) {
  ...
  try {
    const agent = updateAgentCapabilities(
    ...
    const message =
      error instanceof Error ? error.message : "Failed updating capabilities";
    return NextResponse.json(
