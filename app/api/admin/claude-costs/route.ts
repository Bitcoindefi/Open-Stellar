...
import {
  getAgentClaudeAnalytics,
  listClaudeCostRecords,
  ...
  return api.json(
    {
      ...
      records,
      analytics: agentId
        ? getAgentClaudeAnalytics(agentId, dailyBudgetUsd)
        : null,
      alerts:
        agentId &&
        getAgentClaudeAnalytics(agentId, dailyBudgetUsd).overDailyBudget
          ? [
              {
                type: "daily_budget_exceeded",
                agentId,
                dailyBudgetUsd: dailyBudgetUsd ?? 5,
              },
            ]
          : [],
    },
