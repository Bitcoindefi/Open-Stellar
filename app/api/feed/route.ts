import {
  createSystemEventStream,
  eventStreamHeaders,
} from "@/lib/events/event-stream";
import {
...
function streamFeed(req: Request) {
  ...
  const district: DistrictId | null = isDistrictId(requestedDistrict)
    ? requestedDistrict
    : null;
  const baseStream = createSystemEventStream(
