...
const spec = {
  openapi: "3.1.0",
  info: {
    title: "Open Stellar API",
    version: "0.2.0",
    description:
      "Developer API for Open Stellar agents, x402 payments, ZK Passport, reputation, Stellar helpers, feeds, and admin workflows.",
  },
  servers: [
    {
      url: "https://open-stellar.example",
  ...
  paths: {
    "/api/agents/{id}/task": {
      ...
      post: op(
        ...
        ["id"],
        { title: "Inspect feed", payload: { scope: "latest" } },
        {
          successStatus: 201,
          responseSchema: {
            type: "object",
    ...
    "/api/agents/{id}/rate-limit/status": {
      get: op(
        "Agents",
        "Read rate limit usage for an agent",
        ["id"],
        undefined,
        {
          responseSchema: {
    ...
    "/api/agents/{id}/messages": {
      post: op("Agents", "Send a message to an agent", ["id"], {
        role: "user",
        content: "Hello",
      }),
    },
    "/api/agents/{id}/health": {
      get: op("Agents", "Read agent health", ["id"]),
    },
    "/api/agents/{id}/heartbeat": {
      post: op("Agents", "Record agent heartbeat", ["id"], {
        status: "active",
        load: 0.2,
      }),
    },
    ...
    "/api/agents/{id}/appearance": {
      get: op("Agents", "Read agent appearance", ["id"]),
      post: op("Agents", "Update agent appearance", ["id"], {
        skin: "default",
        accessories: [],
      }),
    },
    "/api/agents/{id}/xp/history": {
      get: op("Agents", "Read agent XP history", ["id"], undefined, {
        query: [
          ...
          queryParam("type", {
            type: "string",
            enum: xpHistoryEventSchema.properties.type.enum as string[],
          }),
    ...
    "/api/agents/{id}/credential": {
      post: op("Agents", "Issue a reputation credential", ["id"], {
        contractId: "optional-soroban-contract-id",
      }),
    },
    "/api/agents/{id}/credential/latest": {
      get: op("Agents", "Read latest reputation credential", ["id"]),
    },
    "/api/agents/{id}/webhooks/failures": {
      get: op(
        "Agents",
        "List dead webhook deliveries for an agent",
        ["id"],
        undefined,
        {
          responseSchema: {
    ...
    "/api/badges": {
      get: op(
        "Agents",
        "List badge catalog with earnedByCount",
        [],
        undefined,
        {
          query: [
    ...
    "/api/protocol/x402/quote": {
      get: op("Protocol", "Create an x402 payment quote"),
      post: op("Protocol", "Create an x402 payment quote", [], {
    ...
    "/api/protocol/x402/settle": {
      post: op("Protocol", "Settle an x402 payment", [], {
        quoteId: "quote_123",
        paymentSource: "stellar:testnet",
      }),
    },
    "/api/protocol/passport/authorize": {
      post: op("Protocol", "Authorize a spend with ZK Passport", [], {
        agentId: "agent-nexus",
        amount: "0.25",
        quoteId: "quote_123",
      }),
    },
    "/api/protocol/passport/status": {
      get: op("Protocol", "Lookup passport status"),
    },
    "/api/protocol/reputation": {
      get: op("Protocol", "Query reputation"),
      post: op("Protocol", "Record reputation action", [], {
        actorId: "agent-nexus",
        delta: 1,
        reason: "paid-delivery",
        scope: "tx",
      }),
    },
    "/api/protocol/track8004": {
      get: op("Protocol", "Resolve ERC-8004 style agent identity"),
    },
    "/api/stellar/balance": {
      post: op("Stellar", "Read Stellar balance", [], { publicKey: "G..." }),
    },
    "/api/stellar/build-tx": {
      post: op("Stellar", "Build a Stellar transaction", [], {
        sourcePublic: "G...",
        destination: "G...",
        amount: "1",
      }),
    },
    "/api/stellar/submit-tx": {
      post: op("Stellar", "Submit a signed Stellar transaction", [], {
        xdr: "AAAA...",
      }),
    },
    "/api/stellar/fund": {
      post: op("Stellar", "Fund a testnet account with Friendbot", [], {
        publicKey: "G...",
      }),
    },
    "/api/events": {
      get: op("Events", "List system events"),
      post: op("Events", "Create a system event", [], {
        type: "agent.task.completed",
        agentId: "agent-nexus",
      }),
    },
    "/api/events/{agentId}": {
      get: op("Events", "List events for an agent", ["agentId"]),
    },
    "/api/webhooks": {
      get: op("Webhooks", "List webhook registrations"),
      post: op("Webhooks", "Create a webhook registration", [], {
        url: "https://partner.example/hooks",
        events: ["agent.status"],
      }),
    },
    "/api/webhooks/{id}": {
      delete: op("Webhooks", "Delete a webhook registration", ["id"]),
    },
    "/api/webhooks/{id}/rotate": {
      post: op("Webhooks", "Rotate a webhook signing secret", ["id"]),
    },
    "/api/webhooks/{id}/replay": {
      post: op(
        "Webhooks",
        "Replay webhook events by time range",
        ["id"],
        { from: "2026-06-26T00:00:00.000Z", to: "2026-06-26T01:00:00.000Z" },
        {
          responseSchema: {
            type: "object",
            properties: {
    ...
    "/api/webhooks/event-types": {
      get: op("Webhooks", "List supported webhook event types"),
    },
    ...
    "/api/districts/{districtId}/broadcast": {
      post: op("Events", "Broadcast a district event", ["districtId"], {
        message: "Throughput race started",
      }),
    },
      ...
      get: op("Notifications", "Read notification preferences", [], undefined, {
        ...
        responseSchema: {
          ...
          properties: {
            ...
            muted: {
              type: "array",
              items: { type: "string", enum: [...NOTIFICATION_TYPES] },
            },
    ...
    "/api/quests/{id}/subtasks": {
      post: op(
        "Quests",
        "Create a quest subtask",
        ["id"],
        { title: "Setup service metadata", assignedAgentId: "agent-123" },
        {
          successStatus: 201,
          requestBodySchema: {
            type: "object",
    ...
    "/api/quests/{id}/subtasks/{subtaskId}": {
      patch: op(
        "Quests",
        "Update a quest subtask",
        ["id", "subtaskId"],
        { status: "done", assignedAgentId: "agent-456" },
        {
          requestBodySchema: {
            type: "object",
            properties: {
    ...
    "/api/admin/runs": {
      get: op("Admin", "List orchestration runs"),
      post: op("Admin", "Create an orchestration run", [], {
        title: "Research run",
        steps: [],
      }),
    },
    "/api/admin/runs/{runId}": {
      get: op("Admin", "Read orchestration run", ["runId"]),
      post: op("Admin", "Update or re-run orchestration run", ["runId"], {
        action: "rerun",
      }),
    },
    "/api/user/export": { get: op("User", "Export user data") },
    "/api/user/delete-request": {
      post: op("User", "Request user deletion", [], {
    ...
    responses: {
      [successStatus]: {
        ...
        content: options.responseSchema
          ? { "application/json": { schema: options.responseSchema } }
          : json,
      },
      400: error,
      429: rateLimit,
