...
export async function generateMetadata({
  ...
  if (!credential) {
    return {
      ...
      description:
        "No public reputation credential has been issued for this agent yet.",
    };
  }
      ...
      <main className="min-h-screen bg-[#030712] px-4 py-8 text-slate-100 sm:px-6 lg:px-8">
        <div className="mx-auto flex max-w-3xl flex-col gap-5">
          <Button
            asChild
            variant="outline"
            className="w-fit border-cyan-400/30 bg-cyan-400/10 text-cyan-100 hover:bg-cyan-400/20"
          >
          ...
          <Card className="border-slate-800 bg-slate-950/85 text-slate-100">
            ...
            <CardContent className="text-sm text-slate-400">
              This public page is read-only. A credential must be issued through
              the API before it appears here.
            </CardContent>
          </Card>
        </div>
      </main>
          ...
          <Card className="border-slate-800 bg-slate-950/85 text-slate-100 shadow-[0_24px_80px_rgba(2,8,23,0.45)]">
            ...
            <CardContent className="grid gap-4">
              <div className="grid gap-3 sm:grid-cols-3">
                ...
                <Stat
                  label="Success rate"
                  value={`${credential.agent.successRate.toFixed(1)}%`}
                />
                <Stat
                  label="Reputation"
                  value={`${credential.reputation.score} / ${credential.reputation.tier}`}
                />
              </div>
              ...
              <div className="flex flex-wrap gap-3">
                ...
                {isOnChainIssued && credential.links.explorerUrl ? (
                  <Button asChild variant="outline">
                    <a
                      href={credential.links.explorerUrl}
                      target="_blank"
                      rel="noreferrer"
                    >
                      <ExternalLink />
          ...
          <Card className="border-slate-800 bg-slate-950/85 text-slate-100">
            <CardHeader>
              ...
              <CardTitle className="flex items-center gap-2 text-xl">
                <VerifyIcon
                  className={
                    verification.ok ? "text-emerald-300" : "text-red-300"
                  }
                />
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 rounded-md border border-emerald-400/30 bg-emerald-400/10 px-3 py-2 text-emerald-100">
                ...
                {isOnChainIssued && credential.links.explorerUrl ? (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="inline-flex items-center gap-1 text-cyan-200 underline-offset-4 hover:underline"
                  >
                    Stellar explorer
                  ...
                  <span className="text-slate-400">
                    local attestation{" "}
                    {credential.proof.attestationHash.slice(0, 12)}...
                  </span>
                )}
              </div>
