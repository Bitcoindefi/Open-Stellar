...
export async function generateMetadata({ params }: AgentLeaderboardPageProps) {
  ...
  return {
    title: agent
      ? `${agent.name} Leaderboard Stats | Open Stellar`
      : "Agent not found | Open Stellar",
  };
}

export default async function AgentLeaderboardPage({
  params,
}: AgentLeaderboardPageProps) {
  const { agentId } = await params;
  const agent = getLeaderboardAgent(agentId);
  ...
  const tasks = [
    "Settled x402 receipt",
    "Completed district dispatch",
    "Verified badge proof",
    "Routed agent payment",
  ];
    ...
    <main className="min-h-screen bg-[#030712] px-4 py-8 text-slate-100 sm:px-6 lg:px-8">
      <div className="mx-auto flex max-w-5xl flex-col gap-6">
        ...
        <header className="rounded-2xl border border-slate-800 bg-slate-950/80 p-5">
          ...
          <h1
            className="mt-3 font-pixel text-3xl uppercase"
            style={{ color: agent.districtColor }}
          >
            {agent.name}
          </h1>
          <p className="mt-3 font-mono text-sm text-slate-400">
            Global #{agent.globalRank} / District #{agent.districtRank} / Level{" "}
            {agent.level} / {agent.tasksCompleted.toLocaleString()} completed
            tasks
          </p>
        </header>
        ...
        <section className="grid gap-4 md:grid-cols-4">
          ...
          <Stat
            label="x402 revenue"
            value={`$${agent.x402Revenue.toFixed(2)}`}
          />
          <Stat
            label="Weekly tasks"
            value={agent.weeklyTasks.toLocaleString()}
          />
            ...
            <div className="flex h-44 items-end gap-2 border-b border-l border-slate-700 p-3">
              {rankHistory.map((point, index) => (
                <div
                  ...
                  style={{ height: `${Math.max(12, 100 - point * 12)}%` }}
                  title={`#${point}`}
                />
              ))}
            </div>
...
function Stat({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-2xl border border-slate-800 bg-slate-950/80 p-4">
      ...
      <div className="mt-2 font-pixel text-xl text-slate-100">{value}</div>
    </div>
  );
}
...
function Panel({ title, children }: { title: string; children: ReactNode }) {
  return (
    <section className="rounded-2xl border border-slate-800 bg-slate-950/80 p-5">
      <h2 className="mb-4 font-pixel text-xl uppercase text-slate-100">
        {title}
      </h2>
      {children}
    </section>
