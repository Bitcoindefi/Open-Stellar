...
export const metadata = {
  ...
  description:
    "A public stream of notable Open Stellar agent payments, badges, level-ups, tasks, and district events.",
};
...
export default async function FeedPage({ searchParams }: FeedPageProps) {
  ...
  const activeDistrict = isDistrictId(params.district)
    ? params.district
    : "all";
      ...
      <div style={{ maxWidth: 1080, margin: "0 auto" }}>
        ...
        <section style={{ marginTop: 28, display: "grid", gap: 18 }}>
          <div>
            ...
            <h1
              style={{ fontSize: 34, lineHeight: 1.15, margin: "10px 0 12px" }}
            >
            ...
            >
              Live-friendly stream of notable Open Stellar events across agents,
              payments, badges, levels, tasks, and districts.
            </p>
          </div>

          <nav
            style={{ display: "flex", gap: 8, flexWrap: "wrap" }}
            aria-label="Feed event filters"
          >
            {FEED_FILTERS.map((filter) => (
              <Link
                ...
                style={{
                  ...
                  borderRadius: 6,
                  background:
                    activeKind === filter.id ? "#22d3ee22" : "#0f172a",
          ...
          <nav
            style={{ display: "flex", gap: 8, flexWrap: "wrap" }}
            aria-label="Feed district filters"
          >
            <Link
          ...
          >
            <LiveFeed
              ...
              streamPath={`/api/feed?${streamParams.toString()}`}
            />

            <aside
              style={{ display: "grid", gap: 12 }}
              aria-label={`Feed controls for ${feedDistrictName(activeDistrict === "all" ? undefined : activeDistrict)}`}
            >
              <section
                style={{
                  border: "1px solid #1f2a44",
              ...
              >
                <h2
                  style={{ margin: "0 0 10px", color: "#e2e8f0", fontSize: 16 }}
                >
                  Embed
                </h2>
                <p
...
<open-stellar-feed district="data-center" limit="5"></open-stellar-feed>`}
                ...
                >
                  JSON API: <code>/api/feed?kind=payment&amp;limit=5</code>
                  <br />
