...
export default async function AgentPage({ params }: AgentPageProps) {
  ...
  try {
    const res = await fetch(absoluteUrl(`/api/agents/${id}`), {
      cache: "no-store",
    });
  ...
  try {
    const res = await fetch(absoluteUrl(`/api/agents/${id}/health`), {
      cache: "no-store",
    });
  ...
  try {
    const res = await fetch(
      absoluteUrl(`/api/protocol/reputation?actorId=${id}`),
      { cache: "no-store" },
    );
  ...
  try {
    const res = await fetch(
      absoluteUrl(`/api/agents/${id}/quest-recommendations`),
      { cache: "no-store" },
    );
  ...
  try {
    const res = await fetch(
      absoluteUrl(`/api/agents/${id}/xp/history?pageSize=100`),
      { cache: "no-store" },
    );
  ...
  if (healthData) {
    ...
    runtimeStatus =
      healthData.health?.runtimeStatus ||
  ...
  if (reputationData) {
    ...
    badges =
      reputationData.reputation?.metrics?.badges ||
      reputationData.reputation?.badges ||
      [];
    infractions =
      reputationData.reputation?.metrics?.infractions ||
      reputationData.reputation?.history?.filter((h: any) => h.delta < 0)
        .length ||
      0;
  } else {
  ...
  if (xpHistoryData) {
    xpHistoryEvents = Array.isArray(xpHistoryData.events)
      ? xpHistoryData.events
      : [];
  } else {
          ...
          <CardHeader className="flex flex-col sm:flex-row items-center gap-6 pb-6">
            ...
            <div className="flex flex-col gap-2 items-center sm:items-start flex-1 text-center sm:text-left">
              <div className="flex flex-col sm:flex-row items-center gap-3">
                <h1
                  className="font-pixel text-2xl sm:text-3xl uppercase text-slate-100"
                  style={{ color: localAgent?.color }}
                >
                  {agentName}
                </h1>
                <div className="flex flex-wrap gap-2">
                  ...
                  <Badge
                    variant="outline"
                    className={getStatusBadgeStyle(runtimeStatus)}
                  >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {badges.length > 0 ? (
                    badges.map((badge, i) => (
                      <div
                        key={i}
                        className={`flex flex-col p-3 rounded-lg border gap-1.5 ${getBadgeRarityStyles(badge.rarity)}`}
                      >
                        <div className="flex items-center justify-between">
                          <span className="font-pixel text-xs leading-tight text-slate-100">
                            {badge.name || badge.badgeId || badge.id}
                          </span>
                          <span className="font-mono text-[9px] uppercase px-1.5 py-0.5 rounded border border-current opacity-80">
                            {badge.rarity || "common"}
                          </span>
                        </div>
                        {badge.description && (
                    ...
                    <div className="col-span-1 sm:col-span-2 lg:col-span-3 text-center p-4">
                      <span className="text-sm text-slate-500 font-mono">
                        No badges earned yet. Complete daily quests to earn
                        badges!
                      </span>
                    </div>
                  )}
