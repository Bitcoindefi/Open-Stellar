...
import type { SanitizedApiKey } from "@/lib/auth/api-keys"

const AVAILABLE_SCOPES = [
  { id: "x402:quote", label: "x402:quote", desc: "Create and fetch pricing quotes for payments" },
  { id: "x402:settle", label: "x402:settle", desc: "Settle blockchain payments and claim receipts" },
  { id: "agents:read", label: "agents:read", desc: "Read public agent registry and capabilities" },
  { id: "agents:write", label: "agents:write", desc: "Register, modify, or configure cloud agents" },
  { id: "*", label: "* (Full Access)", desc: "Full administrative permissions across all endpoints" },
]
...
export default function AdminKeysPage() {
  ...
  const handleRevokeKey = async (id: string) => {
    if (!confirm("Are you sure you want to immediately revoke this API key?")) return
  ...
  const handleRotateKey = async (id: string, name: string) => {
    if (!confirm("Rotating will immediately invalidate the current secret and generate a new one. Continue?")) {
      return
    }
  ...
  const totalRequestsCount = keys.reduce((acc, k) => acc + (k.requestCount || 0), 0)
      ...
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="font-vt323 text-lg text-slate-400">
                Issue scoped machine-to-machine tokens, configure rate limits, and enforce zero-trust production auth.
              </p>
            </div>
          </div>
        ...
        <div className="mb-8 grid grid-cols-1 gap-4 sm:grid-cols-3">
          <div className="rounded-2xl border border-slate-800/80 bg-slate-950/60 p-5 backdrop-blur">
            <div className="flex items-center justify-between">
              <span className="font-vt323 text-lg uppercase tracking-wider text-slate-400">Active Keys</span>
              <Shield className="h-5 w-5 text-emerald-400" />
            </div>
            <div className="mt-2 flex items-baseline gap-2">
              <span className="font-mono text-3xl font-bold text-white">{activeKeysCount}</span>
          ...
          <div className="rounded-2xl border border-slate-800/80 bg-slate-950/60 p-5 backdrop-blur">
            <div className="flex items-center justify-between">
              <span className="font-vt323 text-lg uppercase tracking-wider text-slate-400">Total API Traffic</span>
              <Zap className="h-5 w-5 text-amber-400" />
            </div>
            <div className="mt-2 flex items-baseline gap-2">
              <span className="font-mono text-3xl font-bold text-white">{totalRequestsCount}</span>
          ...
          <div className="rounded-2xl border border-slate-800/80 bg-slate-950/60 p-5 backdrop-blur">
            <div className="flex items-center justify-between">
              <span className="font-vt323 text-lg uppercase tracking-wider text-slate-400">Storage Security</span>
              <Lock className="h-5 w-5 text-cyan-400" />
            </div>
            <div className="mt-2 flex items-baseline gap-2">
              <span className="font-mono text-lg font-bold text-cyan-300">SHA-256 Hashed</span>
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="mt-1 text-sm text-slate-300">
                  Please copy and save this secret key now. For your security, <strong className="text-amber-300">it will never be displayed again</strong>.
                </p>
        ...
        {isCreating && (
          <div className="mb-8 rounded-2xl border border-cyan-500/30 bg-slate-950/80 p-6 backdrop-blur">
            ...
            <p className="font-vt323 text-lg text-slate-400">
              Provision credentials with granular service access and sliding-window rate limit tiers.
            </p>
                    ...
                    >
                      <div className="flex items-center gap-2">
                        <input
                          ...
                          className="text-cyan-500 focus:ring-0"
                        />
                        <span className="font-mono text-sm font-semibold">Pro</span>
                      </div>
                      <span className="text-xs text-slate-400">600 req/min</span>
                    </label>
                  </div>
                </div>
                ...
                <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
                  {AVAILABLE_SCOPES.map((scope) => {
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center justify-between">
                          <span className="font-mono text-xs font-bold text-cyan-300">{scope.label}</span>
                          <input
                            type="checkbox"
                            checked={isSelected}
                            onChange={() => {}}
                        ...
                        <p className="mt-1 text-[11px] text-slate-400">{scope.desc}</p>
                      </div>
                    )
                  })}
                </div>
        ...
        <div className="overflow-hidden rounded-2xl border border-slate-800/80 bg-slate-950/60 backdrop-blur">
          ...
          {loading ? (
            ...
            <div className="py-16 text-center">
              ...
              <h3 className="mt-3 font-pixel text-lg text-slate-400">No API Keys Issued Yet</h3>
              <p className="font-vt323 text-lg text-slate-500">
                Click &quot;Create API Key&quot; above to issue your first machine token.
              </p>
            </div>
          ) : (
                  ...
                  {keys.map((key) => (
                    <tr key={key.id} className="hover:bg-slate-900/20">
                      <td className="px-6 py-4">
                        ...
                        <div className="mt-0.5 font-mono text-xs text-cyan-400/90">{key.keyPrefix}</div>
                      </td>
