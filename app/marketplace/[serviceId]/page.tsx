...
export function generateStaticParams() {
  return listMarketplaceServices().map((service) => ({
...
export async function generateMetadata({
  ...
  return {
    title: service
      ? `${service.name} | Open Stellar Marketplace`
      : "Service not found",
  ...
  return (
    <main className="min-h-screen bg-[#030712] px-4 py-8 text-slate-100 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-6xl space-y-6">
        <Link
          href="/marketplace"
          className="inline-flex items-center gap-2 font-mono text-xs uppercase tracking-[0.22em] text-cyan-200 hover:text-cyan-100"
        >
            ...
            <div className="flex gap-4">
              ...
              <div>
                <div className="mb-2 flex flex-wrap gap-2">
                  ...
                  <span
                    className={`rounded-full px-2 py-1 text-xs ${service.status === "online" ? "bg-emerald-400/10 text-emerald-300" : "bg-slate-700 text-slate-300"}`}
                  >
                    {service.status}
                  </span>
                </div>
                <h1 className="font-mono text-3xl font-bold uppercase text-cyan-100 md:text-5xl">
                  {service.name}
                </h1>
        ...
        <section className="grid gap-5 lg:grid-cols-2">
          <Panel title="One-click copy" icon={<Copy className="h-4 w-4" />}>
            <pre className="overflow-x-auto rounded-2xl bg-black/50 p-4 text-sm leading-6 text-slate-200">
              <code>{installSnippet}</code>
            </pre>
            <pre className="mt-3 overflow-x-auto rounded-2xl bg-black/50 p-4 text-sm leading-6 text-slate-200">
        ...
        <section className="grid gap-5 lg:grid-cols-2">
          <Panel title="Example request / response">
            <div className="grid gap-3 md:grid-cols-2">
              <pre className="overflow-x-auto rounded-2xl bg-black/50 p-4 text-xs leading-5 text-slate-200">
                <code>{JSON.stringify(service.exampleRequest, null, 2)}</code>
              </pre>
              <pre className="overflow-x-auto rounded-2xl bg-black/50 p-4 text-xs leading-5 text-slate-200">
                <code>{JSON.stringify(service.exampleResponse, null, 2)}</code>
              </pre>
            </div>
          </Panel>
              ...
              {service.uptime.map((point) => (
                ...
                >
                  <div
                    ...
                    style={{ height: `${Math.max(8, point.value - 90) * 10}%` }}
                  />
              ...
              <tbody>
                {service.receiptHistory.map((receipt) => (
                  <tr key={receipt.id} className="border-t border-slate-800">
                    <td className="p-3 font-mono text-cyan-200">
                      {receipt.id}
                    </td>
                    <td className="p-3">{receipt.agent}</td>
                    <td className="p-3 font-mono">
                      {receipt.amountXlm.toFixed(2)} XLM
                    </td>
...
function Panel({
  ...
  return (
    <section className="rounded-3xl border border-slate-800 bg-slate-950/80 p-5">
      <h2 className="mb-4 flex items-center gap-2 font-mono text-sm uppercase tracking-[0.24em] text-cyan-200">
        {icon}
        {title}
      </h2>
      {children}
