...
describe("evaluateFilters", () => {
  ...
  it("ANDs multiple filters", () => {
    ...
    expect(evaluateFilters(filters, { agentId: "bot-1", amount: 150 })).toBe(
      true,
    );
    expect(evaluateFilters(filters, { agentId: "bot-1", amount: 50 })).toBe(
      false,
    );
    expect(evaluateFilters(filters, { agentId: "bot-2", amount: 150 })).toBe(
      false,
    );
  });
});
