...
describe("task-queue", () => {
  ...
  it("creates a task and returns it via dequeue", () => {
    const { task, overflow } = createTask("agent-1", {
      type: "payment",
      payload: { amount: 100 },
    });
  ...
  it("returns overflow (429) on 101st pending task", () => {
    for (let i = 0; i < MAX_PENDING_PER_AGENT; i++) {
      const { overflow } = createTask("agent-1", {
        type: "payment",
        payload: { i },
      });
      expect(overflow).toBe(false);
  ...
  it("does not count running/completed tasks toward pending limit", () => {
    ...
    const { overflow: overflow2 } = createTask("agent-1", {
      type: "payment",
      payload: {},
    });
  ...
  it("updates task status to completed", () => {
    ...
    const updated = updateTask("agent-1", task.id, {
      status: "completed",
      result: { txHash: "abc" },
    });
  ...
  it("returns null when updating non-existent task", () => {
    const updated = updateTask("agent-1", "non-existent", {
      status: "completed",
    });
  ...
  it("throws on invalid status update", () => {
    ...
    expect(() =>
      updateTask("agent-1", task.id, { status: "pending" as any }),
    ).toThrow("status must be 'completed' or 'failed'");
  });

  // ─── getTask / listAgentTasks ──────────────────────────────────────
  it("gets a specific task", () => {
    const { task } = createTask("agent-1", {
      type: "payment",
      payload: { x: 1 },
    });
  ...
  it("isolates tasks between agents", () => {
    const { task: t1 } = createTask("agent-a", {
      type: "payment",
      payload: {},
    });
  ...
  it("throws on empty agentId", () => {
    expect(() => createTask("", { type: "payment", payload: {} })).toThrow(
      "agentId must not be empty",
    );
  });

  it("throws on empty type", () => {
    expect(() => createTask("agent-1", { type: "", payload: {} })).toThrow(
      "task type must not be empty",
    );
  });
