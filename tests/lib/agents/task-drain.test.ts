...
describe("task-queue drain and purge", () => {
  ...
  describe("drainAgentTasks", () => {
    ...
    it("returns 409 on concurrent drain attempts", async () => {
      ...
      const { result: result2, alreadyDraining } =
        await drainAgentTasks("agent-1");
    ...
    it("allows drain after previous drain completes", async () => {
      ...
      const { result: result1, alreadyDraining: drain1 } =
        await drainAgentTasks("agent-1", {
          maxItems: 5,
        });
      ...
      const { result: result2, alreadyDraining: drain2 } =
        await drainAgentTasks("agent-1", {
          maxItems: 5,
        });
